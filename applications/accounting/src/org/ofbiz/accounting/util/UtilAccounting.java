...
public class UtilAccounting {
    ...
    public static BigDecimal getNetBalance(GenericValue account) throws GenericEntityException {
        ...
        BigDecimal postedDebits = (account.getString("postedDebits") == null) ? 
        		bdZero : account.getBigDecimal("postedDebits");
        BigDecimal postedCredits = (account.getString("postedCredits") == null) ? 
        		bdZero : account.getBigDecimal("postedCredits");
        if (isDebitAccount(glAccount)) {
            balance = postedDebits.subtract(postedCredits);
        } else if (isCreditAccount(glAccount)) {
            balance = postedCredits.subtract(postedDebits);
        }
        return balance;
    }
    ...
    /**
     * Metodo que busca las claves presupuestales por medio de un agrupador y un tipo de poliza 
     * @param agrupador
     * @param tipoPolizaId
     * @param delegator
     * @return
     ...
     */
    public static List<GenericValue> obtenerClavesXAgrupador(String agrupador,String tipoPolizaId, String tipoDocAnterior, Delegator delegator) throws GenericEntityException{
    	
    	EntityCondition condiciones = EntityCondition.makeCondition(EntityOperator.AND,
    														EntityCondition.makeCondition("agrupador",EntityOperator.EQUALS,agrupador),
    														EntityCondition.makeCondition("tipoPolizaId",EntityOperator.EQUALS,tipoPolizaId),
    														EntityCondition.makeCondition("claveContabilizada",EntityOperator.EQUALS,"N"),
    														EntityCondition.makeCondition("idTipoDoc",EntityOperator.EQUALS,tipoDocAnterior));
    	
    	List<GenericValue> clavesPresup = delegator.findByCondition("AgrupadorClaves", condiciones, UtilMisc.toList("clavePresupuestal","productId","currencyUomId","postedAmount"), UtilMisc.toList("clavePresupuestal"));
    	
    	return clavesPresup;
    }
    
    /**
     * Metodo que busca el id de poliza de un documento predecesor
     * @param idTipoDoc
     * @param delegator
     * @return
     * @throws GenericEntityException
     */
    public static String buscarIdPolizaXDocu(String idTipoDoc, Delegator delegator) throws GenericEntityException{
    	
		GenericValue tipoDoc = delegator.findByPrimaryKey("TipoDocumento", UtilMisc.toMap("idTipoDoc",idTipoDoc));
		GenericValue tipoDocAnt = tipoDoc.getRelatedOne("tipo_modulo_predeTipoDocumento");
		if(tipoDocAnt != null){
			GenericValue evento = tipoDocAnt.getRelatedOne("Evento");
			
			if(evento.getString("referencia").equalsIgnoreCase("N"))
				return evento.getString("tipoPolizaIdPres");
			else
				return evento.getString("tipoPolizaIdCont");
		} else {
			return "";
		}		
    }
    ...
    /**
     * Metodo que busca el documento anterior
     * @param idTipoDoc
     * @param delegator
     * @return
     * @throws GenericEntityException
     */
    public static String buscarTipoDocAntXDocu(String idTipoDoc, Delegator delegator) throws GenericEntityException{
    	String predecesor = "";
		...
		List<GenericValue> tipoDocActual = delegator.findByCondition("TipoDocumento", conditions, UtilMisc.toList("predecesor"),UtilMisc.toList("idTipoDoc"));

		Iterator<GenericValue> itemsTipoDocActual = tipoDocActual.iterator();
		if(itemsTipoDocActual.hasNext())
		{	GenericValue itemTipoDoc = itemsTipoDocActual.next();
			predecesor = itemTipoDoc.getString("predecesor");			
		}
		return predecesor;
    ...
    /**
     * Metodo que busca el id de poliza del documento actual
     * @param idTipoDoc
     * @param delegator
     * @return
     * @throws GenericEntityException
     */
    public static String buscarIdPolizaXDocuActual(String idTipoDoc, Delegator delegator) throws GenericEntityException{
    	
		GenericValue tipoDoc = delegator.findByPrimaryKey("TipoDocumento", UtilMisc.toMap("idTipoDoc",idTipoDoc));		
		if(tipoDoc != null){
			GenericValue evento = tipoDoc.getRelatedOne("Evento");
