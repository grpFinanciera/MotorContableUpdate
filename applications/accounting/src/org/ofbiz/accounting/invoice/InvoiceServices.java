...
public class InvoiceServices {
    ...
    public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            if (UtilValidate.isEmpty(invoiceId)) {
                ...
                createInvoiceContext.put("acctgTransTypeId",context.get("acctgTransTypeId"));
            ...
            for (GenericValue currentValue : billItems) {
                ...
                createInvoiceItemContext.put("montoRestante", billingAmount.multiply(billingQuantity));
                ...
                for (GenericValue adj : itemAdjustments) {
                        ...
                        if (amount.signum() != 0) {
                            ...
                            createInvoiceItemAdjContext.put("montoRestante", amount);
            ...
            for (Map<String, Object> commissionMap : commList) {
                ...
                try {
                    resMap = dispatcher.runSync("createInvoiceItem", UtilMisc.toMap(
                            ...
                            "quantity",quantity,
                            "amount", elemAmount,
                            "montoRestante", elemAmount.multiply(quantity),
                            "userLogin", userLogin));
    ...
    public static Map<String, Object> createInvoicesFromShipment(DispatchContext dctx, Map<String, Object> context) {
        ...
        GenericValue orden = null;
        
        try {
        	GenericValue ship = delegator.findByPrimaryKey("Shipment", UtilMisc.toMap("shipmentId", shipmentId));
			orden = delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId", ship.getString("primaryOrderId")));
        ...
        try {
        	if((UtilValidate.isEmpty(orden.getString("pagoAnticipadoRestanteOrg")))||orden.getBigDecimal("pagoAnticipadoRestanteOrg").compareTo(BigDecimal.ZERO)<0){
    ...
    public static Map<String, Object> createInvoicesFromShipments(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String acctgTransTypeId = new String();
        ...
        for (String tmpShipmentId : shipmentIds) {
            try {
                ...
                List<GenericValue> eventos = delegator.findByAnd("EventoContable", UtilMisc.toMap("moduloId","TESORERIA_CXP_OR"));
                if(eventos != null && !eventos.isEmpty()){
                	acctgTransTypeId = eventos.get(0).getString("acctgTransTypeId");
                } else {
                	throw new GenericEntityException("Necesita crear al menos un Evento Contable en el modulo de Cuentas por Pagar");
                }
        ...
        for (String orderId : orders) {
            ...
            Map<String, Object> serviceContext = UtilMisc.toMap("orderId", orderId, "billItems", toBillItems, "invoiceId", invoiceId, "eventDate", context.get("eventDate"), "userLogin", context.get("userLogin"),"acctgTransTypeId",acctgTransTypeId);
            try {
    ...
    public static Map<String, Object> createInvoiceFromReturn(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            for (GenericValue item : billItems) {
                ...
                input.put("montoRestante", returnItem.get("returnPrice"));
                ...
                for (GenericValue adjustment : adjustments) {
                    ...
                    input.put("montoRestante", amount.multiply(quantity));
                    input.put("invoiceItemSeqId", "" + invoiceItemSeqId); // turn the int into a string with ("" + int) hack
                    input.put("productId", returnItem.get("productId"));
            ...
            for (GenericValue adjustment : adjustments) {
                ...
                input = UtilMisc.toMap("invoiceId", invoiceId, "invoiceItemTypeId", invoiceItemTypeId, "quantity", BigDecimal.ONE);
                input.put("amount", amount);
                input.put("montoRestante", amount);
                input.put("invoiceItemSeqId", "" + invoiceItemSeqId); // turn the int into a string with ("" + int) hack
                input.put("description", adjustment.get("description"));
                input.put("overrideGlAccountId", adjustment.get("overrideGlAccountId"));
                input.put("taxAuthPartyId", adjustment.get("taxAuthPartyId"));
    ...
    private static BigDecimal calcHeaderAdj(Delegator delegator, GenericValue adj, String invoiceTypeId, String invoiceId, String invoiceItemSeqId,
        ...
        if (adj.get("amount") != null) {
            ...
            if (amount.signum() != 0) {
                ...
                createInvoiceItemContext.put("montoRestante", amount);
    ...
    /**
     * Metodo que crea los items de una orden a partir de un agrupador dado
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String, Object> createInvoiceItemsAgrupador(DispatchContext dctx, Map<String, Object> context){
    	...
    	GenericValue userLogin = (GenericValue) context.get("userLogin");
    	Locale locale = (Locale) context.get("locale");
    	String poliza = (String) context.get("poliza");
    	String invoiceId = (String) context.get("invoiceId");
    	...
    	try {
    		...
    		String tipoPolizaId = UtilAccounting.buscarIdPolizaXDocu(acctgTransTypeId, delegator);
    		Debug.log("Omar-acctgTransTypeId: " + acctgTransTypeId);			
			Debug.log("tipoPolizaId: " + tipoPolizaId);
			String tipoDocAnterior = UtilAccounting.buscarTipoDocAntXDocu(acctgTransTypeId, delegator);
			List<GenericValue> clavesPres = UtilAccounting.obtenerClavesXAgrupador(poliza,tipoPolizaId, tipoDocAnterior, delegator);			
			Debug.log("clavesPres: " + clavesPres);
			Debug.log("Omar-poliza: " + poliza);
			
			
			int i = 1;
			for (GenericValue clavePresupuestal : clavesPres) {
				int clasif = 1;
				GenericValue clavePresup = clavePresupuestal.getRelatedOne("ClavePresupuestal");
				if(clavePresupuestal.getString("productId") == null || clavePresupuestal.getString("productId").trim().equals(""))
				{	return ServiceUtil.returnError("La p\u00f3liza seleccionada contiene claves presupuestales sin producto");					
				}
				...
				inputItem.put("invoiceItemSeqId", formato.format(i));
				inputItem.put("productId",clavePresupuestal.getString("productId"));
				inputItem.put("uomId", clavePresupuestal.getString("currencyUomId"));
				inputItem.put("quantity", BigDecimal.ONE);
				inputItem.put("amount", clavePresupuestal.getBigDecimal("postedAmount"));
				inputItem.put("montoRestante", clavePresupuestal.getBigDecimal("postedAmount"));
				inputItem.put("taxAuthPartyId", null);
				inputItem.put("taxAuthGeoId", null);
				inputItem.put("clasificacion1", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion2", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion3", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion4", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion5", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion6", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion7", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion8", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion9", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion10", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion11", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion12", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion13", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion14", clavePresup.getString("clasificacion"+clasif++));
				inputItem.put("clasificacion15", clavePresup.getString("clasificacion"+clasif++));
    ...
    public Map<String, Object> validarProveedor(DispatchContext dctx, Map<String, Object> context) throws GenericEntityException
    {	Delegator delegator = dctx.getDelegator();
		LocalDispatcher dispatcher = dctx.getDispatcher();
		Debug.log("Entra a validarProveedor");		
		...
		try 
		{	generic = delegator.findByPrimaryKey("Party", UtilMisc.toMap("partyId", partyIdFrom));
			if(generic!=null)
			{	result.put("result", "1");
				Debug.log("Generic es distinto a null");
			}
			else
			{	result.put("result", "ERROR");
				Debug.log("Generic es igual a null");
			}
