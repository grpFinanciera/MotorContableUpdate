...
public class ShoppingCartEvents {
     ...
     * @throws GenericEntityException */
    public static String clearCart(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        ShoppingCart cart = getCartObject(request);
        Debug.log("ShopjigCarttt: " + cart.cartLines.size());
        /*for (int i = 0; i < cart.cartLines.size(); i++) {
            ShoppingCartItem sci = (ShoppingCartItem) cart.cartLines.get(i);
            if(sci.getDetalleRequisicionId() != null) {
            	ShoppingCartHelper.updateDetalleRequisicion(cart.getDelegator(),
            		sci.getDetalleRequisicionId() , sci.getRequisicionId(), "PENDIENTE_P");
            	Debug.log("ITEM: " + sci.getDetalleRequisicionId());
            }
        }*/
        cart.clear();
     ...
     * @throws GenericEntityException */
    public static String destroyCart(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        HttpSession session = request.getSession();
    ...
    public static String createQuoteFromCart(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        HttpSession session = request.getSession();
        GenericValue userLogin = (GenericValue)session.getAttribute("userLogin");
    ...
    public static String createCustRequestFromCart(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        HttpSession session = request.getSession();
        GenericValue userLogin = (GenericValue)session.getAttribute("userLogin");
        String destroyCart = request.getParameter("destroyCart");
    ...
    public static String bulkAddProductsInApprovedOrder(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        Locale locale = UtilHttp.getLocale(request);
        String productId = null;
