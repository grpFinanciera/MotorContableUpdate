...
import org.ofbiz.webapp.event.UtilServletHandler;
...
public class CheckOutEvents {
    ...
    public static String createOrder(HttpServletRequest request, HttpServletResponse response) {
    	Debug.log("Mike Ingresa.- createOrder");
        HttpSession session = request.getSession();
        ...
        CheckOutHelper checkOutHelper = new CheckOutHelper(dispatcher, delegator, cart);
        String ciclo = UtilServletHandler.getCicloId(request);
        Map callResult;
        ...
        callResult = checkOutHelper.createOrder(userLogin, distributorId, affiliateId, trackingCodeOrders, areOrderItemsExploded, visitId, webSiteId,ciclo);
        if (callResult != null) {
            ServiceUtil.getMessages(request, callResult, null);
            if (ServiceUtil.isError(callResult)) {
