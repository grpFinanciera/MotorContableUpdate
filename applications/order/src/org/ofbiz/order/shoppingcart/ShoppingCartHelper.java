...
public class ShoppingCartHelper {
        ...
        try {
            ...
            if (productId != null) {
                       ...
                       itemId = cart.addOrIncreaseItem(productId, amount, quantity, reservStart, reservLength,
                                                ...
                                                catalogId, configWrapper, itemType, itemGroupNumber, pProductId, dispatcher, null, null, null,null,null,null);
                ...
                if (UtilValidate.isNotEmpty(productId) && orderItem.get("quantity") != null) {
                    ...
                    try {
                        this.cart.addOrIncreaseItem(UtilValidate.isNotEmpty(aggregatedProdId) ? aggregatedProdId :  productId, amount, orderItem.getBigDecimal("quantity"),
                                null, null, null, null, null, null, null, catalogId, configWrapper, orderItemTypeId, itemGroupNumber, null, dispatcher, null, null, null,null,null,null);
                        noItems = false;
                    } catch (CartItemModifyException e) {
                        errorMsgs.add(e.getMessage());
                    } catch (ItemNotFoundException e) {
            ...
            if (UtilValidate.isNotEmpty(quantStr)) {
                ...
                if (quantity.compareTo(BigDecimal.ZERO) > 0) {
                    try {
                        ...
                        this.cart.addOrIncreaseItem(productId, null, quantity, null, null, null, null,
                        		null, null, null, catalogId, null, null, itemGroupNumberToUse, null, dispatcher, null, null, null,null,null,null);
                    } catch (CartItemModifyException e) {
                    ...
                    if (quantity.compareTo(BigDecimal.ZERO) > 0) {
                        ...
                        try {
                            ...
                            int index = this.cart.addOrIncreaseItem(productId, null, quantity, null, null, null,
                            		requirement.getTimestamp("requiredByDate"), null, null, null, catalogId, null,
                            		null, itemGroupNumber, null, dispatcher, null, null, null,null,null,null);
    ...
    public Map<String, Object> modifyCart(Security security, GenericValue userLogin, Map<String, Object> context, boolean removeSelected, String[] selectedItems, Locale locale) throws GenericEntityException {
        Map<String, Object> result = null;
        if (locale == null) {
            locale = this.cart.getLocale();
        }

        List<ShoppingCartItem> deleteList = FastList.newInstance();
        ...
        Debug.log("Entra a cambiar el precio");
        ...
        while (parameterNameIter.hasNext()) {
            ...
            if (underscorePos >= 0) {
                try {
                    ...
                    if (parameterName.toUpperCase().startsWith("OPTION")) {
                    ...
                    } else if (parameterName.toUpperCase().startsWith("UPDATE")) {
                    ...
                    } else if(parameterName.toUpperCase().startsWith(UtilClavePresupuestal.VIEW_TAG_PREFIX.toUpperCase())){
                    	String[] prefijo = parameterName.split(UtilClavePresupuestal.VIEW_TAG_PREFIX);
                    	String[] numero = prefijo[1].split("_");
                    	item.setAttribute(UtilClavePresupuestal.TAG_PARAM_PREFIX+numero[0], quantString);
                    	
                    	//Se arma la clave para verificar su existencia
                    	StringBuffer clavePresupuestal = new StringBuffer();
                    	for (int i = 1; i <= UtilClavePresupuestal.TAG_COUNT; i++) {
                    		String clasificacion = (String) item.getAttribute(UtilClavePresupuestal.TAG_PARAM_PREFIX+i);
                    	...
                    	//Se obtiene el monto para validar el presupuesto
                    	BigDecimal quantityL = BigDecimal.valueOf( Double.valueOf(
                    			(String) (UtilValidate.isEmpty(context.get("update_"+indexStr)) ? 
                    					(UtilValidate.isEmpty(item.getQuantity()) ? "0" : item.getQuantity()) : context.get("update_"+indexStr)) ));
                    	BigDecimal priceL = BigDecimal.valueOf( Double.valueOf(
                    			(String) (UtilValidate.isEmpty(context.get("price_o_"+indexStr)) ? 
                    					(UtilValidate.isEmpty(item.getSelectedAmount()) ? "0" : item.getSelectedAmount()) : context.get("price_o_"+indexStr)) ));
                    	BigDecimal monto = quantityL.multiply(priceL);
                    	
                    	Calendar calFechaPresup = Calendar.getInstance();
                    	calFechaPresup.setTimeInMillis(this.cart.getShipAfterDate().getTime());
                    	calFechaPresup.set(Calendar.MONTH, Integer.valueOf(item.getMesId()) -1);
                    	Timestamp fechaPresup = new Timestamp(calFechaPresup.getTimeInMillis());
                    	
                    	UtilClavePresupuestal.validaSuficienciaExistenciaClave(clavePresupuestal.toString(), monto, 
                    			UtilClavePresupuestal.DISPONIBLE, delegator, fechaPresup, UtilClavePresupuestal.EGRESO_TAG);
                    }
                    ...
                    if (parameterName.toUpperCase().startsWith("PRICE")) {
                        ...
                        quantity =new BigDecimal(quantString);
                        String tmpQuantity = pf.format(quantity);
                        String tmpOldPrice = pf.format(oldPrice);
                        Debug.log("tmpOldPrice: " + tmpOldPrice);
                        if (!tmpOldPrice.equals(tmpQuantity)) {
                            if (security.hasEntityPermission("ORDERMGR", "_CREATE", userLogin)) {
                                if (item != null) {
                                	Debug.log("price: " + quantity);
                                	Debug.log("getBasePrice: " + item.getBasePrice());
                                	if(quantity.compareTo(item.getBasePrice()) != 0){
                                		Debug.log("entra2");
                                		item.setBasePrice(BigDecimal.ZERO);
                                		item.setDisplayPrice(BigDecimal.ZERO);
                                		...
                                		item.setSelectedAmount(quantity);
                                	}else{
                                		Debug.log("entra1");
                                		item.setBasePrice(quantity.ONE); // this is quantity because the parsed number variable is the same as quantity
                                		item.setDisplayPrice(quantity.ONE); // or the amount shown the cart items page won't be right
                                	}
                    ...
                    return ServiceUtil.returnError(nfe.getMessage());
                } catch (Exception e) {
        ...
        while (di.hasNext()) {
            ...
            String detalle = item.getDetalleRequisicionId();
            String requisicion = item.getRequisicionId();
            Debug.log("Item detalleRequisicionId: " + detalle);
            Debug.log("Item requisicionId: " + requisicion);
...
//                if(detalle != null && requisicion != null) {
//                	updateDetalleRequisicion(delegator, detalle, requisicion, "PENDIENTE_P");
//                }
            } catch (CartItemModifyException e) {
    ...
    /*
	 * Actualizar producto de la requisicion
	 */
	public static void updateDetalleRequisicion(Delegator delegator,
			String detalleRequisicionId, String requisicionId, String estatus)
			throws GenericEntityException {

		GenericValue deta = delegator.findByPrimaryKey("DetalleRequisicion",
				UtilMisc.toMap("detalleRequisicionId", detalleRequisicionId,
						"requisicionId", requisicionId));
		Debug.log("Trajo..... " + deta);
		if (!deta.isEmpty()
				&& !deta.getString("estatusProducto").equals("ORDENADO")) {
			...
			detalle.set("estatusProducto", estatus);
			delegator.create(detalle);
		}
	}
