...
public class CheckOutHelper {
    ...
    public Map<String, Object> createOrder(GenericValue userLogin) {
        return createOrder(userLogin, null, null, null, false, null, null,null);
    }

    // Create order event - uses createOrder service for processing
    public Map<String, Object> createOrder(GenericValue userLogin, String distributorId, String affiliateId,
            List trackingCodeOrders, boolean areOrderItemsExploded, String visitId, String webSiteId, String ciclo) {
    	Debug.log("Mike Ingresa.- createOrder 2");
        if (this.cart == null) {
            return null;
        }
        ...
        context.put("tipoAdjudicacionId", this.cart.getTipoAdjudicacionId());
        context.put("medioId", this.cart.getMedioId());
        context.put("subTipoAdjudicacionId", this.cart.getSubTipoAdjudicacionId());        
        context.put("observacion", this.cart.getObservacion());
        context.put("articuloId", this.cart.getArticuloId());
        context.put("fraccionId", this.cart.getFraccionId());
        context.put("numInvitacion", this.cart.getNumInvitacion());
        context.put("fechaContable", this.cart.getShipAfterDate());
        context.put("garantia", this.cart.getGarantia());
        context.put("ciclo", ciclo);
        context.put("pagoAnticipado", this.cart.getPagoAnticipado());
        context.put("proveedorPagoAnticipado", this.cart.getProveedorPagoAnticipado());
        ...
        try {
            storeResult = dispatcher.runSync("storeOrder", context, 1200, false);
            ...
            if (productId != null) {
                try {
                    ...
                    if ("AGGREGATED_CONF".equals(product.getString("productTypeId"))) {
                        ...
                        if(ServiceUtil.isError(prunResult)){
                        	return ServiceUtil.returnError(ServiceUtil.getErrorMessage(prunResult));
                    	}
                    }
                } catch (Exception e) {
                    String service = e.getMessage();
        ...
        while (shoppingCartItems.hasNext()) {
            ...
            Debug.log("Mike Ingresa.- orderItems" + shoppingCartItem);
