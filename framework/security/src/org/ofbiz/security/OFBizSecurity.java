...
public class OFBizSecurity implements Security {
    ...
    protected static final Map<String, Map<String, String>> simpleRoleEntity = UtilMisc.<String, Map<String, String>>toMap(
    ...
    private static Map<String,Set<String>> mapaGroupPermission = FastMap.newInstance();
    ...
    public Iterator<GenericValue> findUserLoginSecurityGroupByUserLoginId(String userLoginId) {
        ...
        try {
        	EntityCondition condicionUsr = EntityCondition.makeCondition(EntityOperator.AND,
        			EntityCondition.makeCondition("userLoginId", EntityOperator.EQUALS, userLoginId),EntityUtil.getFilterByDateExpr());
        	List<String> orderBy = UtilMisc.toList("groupId");
            collection = delegator.findByCondition("UserLoginSecurityGroup", condicionUsr, null, orderBy);
    ...
    public boolean securityGroupPermissionExists(String groupId, String permission) {
        ...
        try {
	    	...
	    	Set<String> setPermission = FastSet.newInstance();
	    	
	    	if(mapaGroupPermission.containsKey(groupId)){
	    		setPermission = mapaGroupPermission.get(groupId);
	    	} else {
	    		 List<GenericValue> listSGP = delegator.findByAndCache("SecurityGroupPermission", UtilMisc.toMap("groupId", groupId));
	    		 for (GenericValue SecurityGroupPermission : listSGP) {
	    			 setPermission.add(SecurityGroupPermission.getString("permissionId"));
				}
	    		 mapaGroupPermission.put(groupId, setPermission);
	    	}
	    	
	    	for (String Permission : setPermission) {
	    		if(Permission.equalsIgnoreCase(permission)){
	    			return true;
	    		}
			}
	    	
	    	GenericValue SecurityGroupPermission = EntityUtil.getFirst(
	    			delegator.findByAndCache("SecurityGroupPermission", UtilMisc.toMap("groupId", groupId,"permissionId",permission)));
