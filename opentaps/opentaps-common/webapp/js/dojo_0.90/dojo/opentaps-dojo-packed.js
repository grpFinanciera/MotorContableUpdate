	...
	dojo.declare("dojo.data.JsonItemStoreAutoComplete", dojo.data.JsonItemStore, {
		...
		_fetchItems : function (_28b, _28c, _28d) {
			...
			if (_29a) {
				if (this._jsonFileUrl) {
					var _29c = {
						url : self._jsonFileUrl + "?keyword=" + _29a+"&filtro="+this.filtro,
						handleAs : "json-comment-optional"
					};
...
if (!dojo._hasResource["dijit.form.ComboBox"]) {
	...
	dojo.declare("dijit.form.ComboBoxMixin", dijit.form._DropDownTextBox, {
		...
		_selectOption : function (evt) {
			...
			this._doSelect(tgt);
			this.setDescription(String(tgt.item.name));
			this.cambio();
		},
		...
		setDescription : function (description){
			var indice = description.split('(');
			if(indice.length > 0 ){description = indice[0];}
			if(this.hiddenId.id){var id = this.hiddenId.id;}else{var id = this.hiddenId;}
			var divDescription = dojo.byId('description_'+id);
			if(divDescription){divDescription.innerHTML = description;}
		},
		_onArrowClick : function () {
			if (this.disabled) {
				return;
...
opentaps.confirmSubmitAction = function(confirmText, form, textoEspera) {
    var answer = confirm(confirmText);
    if (answer && form) {
    	var as = document.getElementsByTagName('a');
    	for (var int = 0; int < as.length; int++) {
			var a = as[int];
			if(a.href.indexOf('opentaps.confirmAction') != -1 || a.href.indexOf('submit()') != -1){
				a.disabled = true;
				a.innerHTML = textoEspera;
			}
		}
...
opentaps.confirmAction = function(confirmText, href, formName, /*Array*/ params, textoEspera) {
    if (href && href.length > 0) {
        ...
        opentaps.confirmSubmitAction(confirmText, form, textoEspera);
    }
}
...
opentaps.getCurrentPage = function(paginatorName, appName, responseFunction) {
    ...
    opentaps.sendRequest('paginate', {'paginatorName' : paginatorName, 'action' : 'getCurrentPage', 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.getNextPage = function(paginatorName, appName, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('paginate', {'paginatorName' : paginatorName, 'action' : 'getNextPage', 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.getPreviousPage = function(paginatorName, appName, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('paginate', {'paginatorName' : paginatorName, 'action' : 'getPreviousPage', 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.getFirstPage = function(paginatorName, appName, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('paginate', {'paginatorName' : paginatorName, 'action' : 'getFirstPage', 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.getLastPage = function(paginatorName, appName, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('paginate', {'paginatorName' : paginatorName, 'action' : 'getLastPage', 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.getPageNumber = function(paginatorName, appName, pageNumber, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('paginate', {'paginatorName' : paginatorName, 'pageNumber' : pageNumber, 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.changePaginationOrder = function(paginatorName, appName, orderBy, orderByReverse, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('changePaginationOrder', {'paginatorName' : paginatorName, 'orderBy' : orderBy, 'orderByReverse' : orderByReverse, 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.changePaginationViewSize = function(paginatorName, appName, delta, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('changePaginationViewSize', {'paginatorName' : paginatorName, 'delta' : delta, 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.setPaginationViewSize = function(paginatorName, appName, newSize, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('changePaginationViewSize', {'paginatorName' : paginatorName, 'newSize' : newSize, 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}
opentaps.togglePaginationViewAll = function(paginatorName, appName, responseFunction) {
    if (! responseFunction) responseFunction = opentaps.processPaginationResponse;
    opentaps.sendRequest('changePaginationViewSize', {'paginatorName' : paginatorName, 'toggleViewAll' : 'y', 'applicationName' : appName}, function(data) {responseFunction(paginatorName, data)}, {target: 'paginate_' + paginatorName, containerClass: 'paginate_' + paginatorName, size: 28},true,60000);
}


/* auto complete functions */

opentaps.initInputAutoCompleteQuick = function(/* String */ jsonItemStoreUrl, /* String */ elementName, /* String */ elementId, /* String */ defaultValue,/*String*/filtro) {
    var store = new dojo.data.JsonItemStoreAutoComplete({url: jsonItemStoreUrl});
    var comboBox = new dijit.form.ComboBox({
    	id:"ComboBox_"+elementId,
    	hiddenId:elementId,
        name:"ComboBox_"+elementName,
        value: defaultValue,
        autoComplete: false,
        store: store,
        searchAttr: "name",
...
function fieldLookup1autocomplete(_targetCombo, _targetHidden, args) {
  ...
  target2 = document.getElementById('description_'+targetHidden.id);;  
...
function cambiaEventoSencillo(/*String*/acctgTransTypeId,/*String*/colspan,/*String*/indice){
	var sufijo = "_o_"+indice;
	var requestData = {'acctgTransTypeId' : acctgTransTypeId};	
	opentaps.sendRequest('getLineasMotor', requestData, function(data) {cargarCuentasResponseOrigen(data,colspan,sufijo)});
};


function cambiaEvento(/*String*/acctgTransTypeId,/*String*/colspan){
	var requestData = {'acctgTransTypeId' : acctgTransTypeId};	
	opentaps.sendRequest('getLineasMotor', requestData, function(data) {cargarCuentasResponseOrigen(data,colspan,'')});
};

function cambiaEventoTodos(/*String*/acctgTransTypeId,/*String*/colspan){
	var requestData = {'acctgTransTypeId' : acctgTransTypeId, 'mostrarTodos' : 'Y'};	
	opentaps.sendRequest('getLineasMotor', requestData, function(data) {cargarCuentasResponseOrigen(data,colspan,'')});
};

function cargarCuentasResponseOrigen(data,/*String*/colspan,/*String*/sufijo){
	limpiarItems(sufijo);
		for (var key in data) 
		{	if(data[key]!="ERROR")
			{	var nombreElem = "item"+sufijo;
				var renglon = document.getElementsByName(nombreElem);
				if(data[key].length > 0){
					for(var j = 0; j < renglon.length; j++){
						var yaPasoPresup = false;
						var yaPasoCont = false;
						var renglonActual = renglon[j];
						var idMuestra = "muestra"+j;
						var muestra = document.getElementById(idMuestra);
						if(muestra && muestra.value=="false"){
							continue;
						}
						//Se crea el nuevo renglon que contendra la tabla 
						var nuevoRenglon = document.createElement("tr");
						var nombreNuevoRenglon = "nuevoRenglon"+sufijo;
						nuevoRenglon.setAttribute("name",nombreNuevoRenglon);
						nuevoRenglon.style.textAlign="right";
						nuevoRenglon.className = renglonActual.className;
						//Se crea columna vacia para crear espacio
						var restaColspan = Number(colspan)-6;
						if(restaColspan>=0){
							var columnaVacia = document.createElement("td");
							columnaVacia.colSpan = restaColspan;
							nuevoRenglon.appendChild(columnaVacia);
						}
						var columnaContenedora = document.createElement("td");
						columnaContenedora.colSpan = "6";
						//Se crea la nueva columna que contendra la tabla
						var columnaContenedora = document.createElement("td");
						columnaContenedora.colSpan = colspan;
						columnaContenedora.style.textAlign="right";
						//Se crea la tabla que contendra los datos a ingresar de la linea contable
						var tablaInterna = document.createElement("table");
						tablaInterna.align="right";
						//Se iteran las lineas contables a ingresar
						for(var i = 0 ; i<data[key].length ; i++){
							var objetoLinea = data[key][i];
							var descripcionSE = objetoLinea.descripcion;
							descripcionSE = descripcionSE.replace(/\s/g,'');
							var renglonLinea = document.createElement("tr");
							var colMonto = document.createElement("td");
							var sufijoCatalogo = "";
							if(objetoLinea.momentoCompara){
								sufijoCatalogo = "P";
								if(!yaPasoPresup){
									agregaTitulo(tablaInterna,"Presupuesto");
									yaPasoPresup = true;
								}
								var colDescripcion = opentaps.createTableCell("","tableheadtext",null,"right","");
								renglonLinea.appendChild(colDescripcion);
							} else {
								sufijoCatalogo = "C";
								if(!yaPasoCont){
									agregaTitulo(tablaInterna,"Patrimonial");
									yaPasoCont = true;
								}
								var colDescripcion = opentaps.createTableCell("","tableheadtext",objetoLinea.descripcion,"right",null);
								renglonLinea.appendChild(colDescripcion);
								var input_name = "monto"+descripcionSE+j+sufijo;
								var inputMonto = opentaps.createInput("",input_name,"text","inputBox",null,null,"15"); 
								inputMonto.onchange = function() {validaFormatoNumero(this)};
								colMonto.appendChild(inputMonto);
							}
							renglonLinea.appendChild(colMonto);
							if(objetoLinea.catalogoCargo){
								var id = "catalogoCargo"+sufijoCatalogo+descripcionSE+j+sufijo;
								agregarCatalogo(renglonLinea,id,
										objetoLinea.catalogoCargo.descripcion,objetoLinea.catalogoCargo.urlBusqueda,
										objetoLinea.catalogoCargo.tipoEntidad);
							}
							if(objetoLinea.catalogoAbono){
								var id = "catalogoAbono"+sufijoCatalogo+descripcionSE+j+sufijo;
								agregarCatalogo(renglonLinea,id,
										objetoLinea.catalogoAbono.descripcion,objetoLinea.catalogoAbono.urlBusqueda,
										objetoLinea.catalogoAbono.tipoEntidad);
							}							
							tablaInterna.appendChild(renglonLinea);
						}
						columnaContenedora.appendChild(tablaInterna);
						nuevoRenglon.appendChild(columnaContenedora);
						opentaps.insertAfter(nuevoRenglon,renglonActual);
					}
				}
			}	    		
		}
...
function agregarCatalogo(/*Element Row*/renglonLinea, /*String*/ id ,/*String*/ descripcion ,/*String*/ urlBusqueda,/*String*/tipoEntidad){
	var colTextoCargo = opentaps.createTableCell("","tableheadtext",descripcion,"right",null);
	renglonLinea.appendChild(colTextoCargo);
	var colCargo = document.createElement("td");
	objetoAutocompletar = opentaps.initInputAutoCompleteQuick(urlBusqueda,id,id,"",tipoEntidad);
	colCargo.appendChild(objetoAutocompletar.domNode);
	var inputHidden = opentaps.createInput(id,id,"hidden","inputBox",null,null,null); 
	colCargo.appendChild(inputHidden);
	var spanCargo = opentaps.createSpan("description_"+id,null,"tooltip", null);
	colCargo.appendChild(spanCargo);
	renglonLinea.appendChild(colCargo);
};

function agregaTitulo(/*Element Table*/tablaInterna, /*String*/ titulo){
	var renglonTexto = document.createElement("tr");
	var colTitulo = opentaps.createTableCell(null, "tableheadtext", titulo, "center","6");
	renglonTexto.appendChild(colTitulo);
	var renglonLinea = document.createElement("tr");
	var colLinea = opentaps.createTableCell(null, "tableheadtext", "<hr>", "center","6");
	renglonLinea.appendChild(colLinea);
	tablaInterna.appendChild(renglonTexto);
	tablaInterna.appendChild(renglonLinea);
};

function limpiarItems(/*String*/sufijo){
	var nombreElem = "nuevoRenglon"+sufijo;
	var elementos = document.getElementsByName(nombreElem);
		while(elementos.length > 0){
			elementos[0].parentNode.removeChild(elementos[0]);
		}
};

function cambiarValorCheckBox(elemento){
	if(elemento.value=="Y"){
		elemento.value="N"
	}else{
		elemento.value="Y"
...
function obtenMesCicloSimple(/*String*/ campoSequence){
	
	var sequenceId = campoSequence.value;
	
	var requestData = {'sequenceId' : sequenceId};	
	opentaps.sendRequest('obtenMesCicloJSON', requestData, function(data) {cargaMesesCiclo(data)});

}

function cargaMesesCiclo(data){
	var select = document.getElementById('customTimePeriodId');
	var hoy = new Date();
...
//Funciones para Autorizar, Comentar o Rechazar una solicitud de cualquier indole (WorkFlow)
function accionWorkFlow(accion , estatus) 
{	confirmar=confirm("\u00bfEst\u00e1 seguro?"); 
	...
	{	var comentario = "";
		...
		var entidadId = document.getElementsByName('entidadId')[0].value;
		var tipoWorkFlowId = document.getElementsByName('tipoWorkFlowId')[0].value;
		var urlHost = document.getElementsByName('urlHost')[0].value;
		var entidad = document.getElementsByName('entidad')[0].value;
		var avisoTesorero = document.getElementsByName('avisoTesorero')[0].value;
//		var form = document.getElementById('accionWorkFlow');
		if (accion != "" && estatus != "" && entidadId != "" && tipoWorkFlowId != "" && urlHost != "" && entidad != "") 
	    { 	
	    	var requestData = {'accion' : accion, 'comentario' : comentario, 'estatus' : estatus, 'entidadId' : entidadId, 'tipoWorkFlowId' : tipoWorkFlowId, 'urlHost' : urlHost, 'entidad' : entidad, 'avisoTesorero' : avisoTesorero};		    			    
	    	opentaps.sendRequest('accionWorkFlow', requestData, function(data) {responseWorkFlow(data, accion)});
	    }else{
	    	alert("Existe un elemento faltante, consulte al administrador");
	    }	        					
	}					
};

	//Functions load the response from the server
	function responseWorkFlow(data, accion) 
	{	for (var key in data) 
		{	if(data[key].indexOf("ERROR") != -1)
			{	
				...
				if(Error[1]){
					...
					alert("Ocurri\u00f3 un error al "+ accion +" los campos");
				}
			}
			else
			{	
				...
				switch(accion){
					case "autorizar":
						mensajeAccion = "autoriz\u00F3";
						break;
					case "rechazar":
						mensajeAccion = "rechaz\u00F3";
						break;
					case "comentar":
						mensajeAccion = "coment\u00F3";
						break;
				}
				alert("Se "+ mensajeAccion +" correctamente");	
				document.getElementById('accionWorkFlow').submit();
			}	    		
		}		
	};
