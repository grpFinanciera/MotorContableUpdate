...
<table class="listTable">
    <tr style="vertical-align:bottom">
        ...
        <th class="rightAlign">${uiLabelMap.ProductosPendientes}</th>
        <th class="rightAlign">${uiLabelMap.CantidadFaltante}</th>
    </tr>
    ...
    <tr class="${tableRowClass(order_index)}">
        ...
        <@displayCell text=order.cantidadFaltante?if_exists />
        <@displayCurrencyCell currencyUomId=order.currencyUom?if_exists amount=order.precioFaltante?default(0.0) />
    </tr>
    </#list>
</table>
