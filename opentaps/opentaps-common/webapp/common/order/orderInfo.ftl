...
<#if (order.isOpen() && !order.isApproved() && (order.isSalesOrder() || security.hasEntityPermission("PRCH", "_ORD_APPRV", session))) >
...
<#--  Imprimir orden o pedido Pentaho -->

<#assign printAction><@actionForm form="orderPrintAction" text="${uiLabelMap.OpentapsImprimirOrden}"/></#assign>
  

<#if order.statusId == "ORDER_CREATED" || order.statusId == "ORDER_COMPLETED" || order.statusId =="ORDER_APPROVED" || order.statusId =="ORDER_CANCELLED" >
	<@form name="verPoliza" url="viewAcctgTrans" acctgTransId=order.acctgTransId />
	<#assign verPolizaAction><@actionForm form="verPoliza" text=uiLabelMap.FinancialVerPoliza /></#assign>
</#if> 

<#if order.statusId == "ORDER_CANCELLED" >
	<@form name="verPolizaCancelada" url="viewAcctgTrans" acctgTransId=order.acctgTransIdCancelada! />
	<#assign verPolizaCanceladaAction><@actionForm form="verPolizaCancelada" text=uiLabelMap.FinancialVerPolizaL /></#assign>
</#if> 

<#--  Actualizar documentacion requerida -->
<#assign docRequeridosAction><@actionForm form="docRequeridos" text="${uiLabelMap.PurchDocumentosPendientes}"/></#assign>
<@form name="docRequeridos" url="documentosPendientes" orderId=order.orderId />
...
<#assign extraOptions>
  <@selectActionForm name="orderActions" prompt="${uiLabelMap.CommonSelectOne}">
    ...
    ${verPolizaCanceladaAction?if_exists}
    ${docRequeridosAction?if_exists}
    ${printAction?if_exists}
   </@selectActionForm>
</#assign>
...
<@frameSection title="${uiLabelMap.OrderOrder} #${order.orderId} ${externalOrder?if_exists} ${uiLabelMap.CommonInformation}" extra=extraOptions>
    <table width="100%" border="0" cellpadding="1" cellspacing="0">
      ...
      <#if order.tipoAdjudicacionId?exists>
      	<@infoRow title=uiLabelMap.ProcesoAdjudicacion content=Static["org.opentaps.common.util.UtilCommon"].getDescripcionTipoAdjudicacion(order.tipoAdjudicacionId!'', delegator)/>
      </#if>
      <#-- Sub tipo de adjudicacion --> 
      <#if order.subTipoAdjudicacionId?exists>
      	<@infoRow title=uiLabelMap.SubProcesoAdjudicacion content=Static["org.opentaps.common.util.UtilCommon"].getDescripcionSubTipoAdjudicacion(order.subTipoAdjudicacionId, order.tipoAdjudicacionId, delegator)/>
      </#if>
      <#if order.articuloId?has_content>
      	<@infoRow title=uiLabelMap.PurchPedidoItemId content=Static["org.opentaps.common.util.UtilCommon"].getDescripcionArticulo(order.tipoAdjudicacionId, order.articuloId, delegator)/>
      </#if>
      <#if order.fraccionId?has_content>
      	<@infoRow title=uiLabelMap.PurchFraccion content=Static["org.opentaps.common.util.UtilCommon"].getDescripcionFraccion(order.fraccionId, delegator)/>
      </#if>
      <#if order.observacion?has_content>
     	 <@infoSepBar/>
      	 <#-- numero de contrato -->
      	 <@infoRow title=uiLabelMap.NumContrato content=order.observacion />
      </#if>
      <@infoSepBar/> 
