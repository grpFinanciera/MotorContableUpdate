          ...
          <#list order.items as item>
           <#if !(item.isCancelled() && item.isPromo())>
            ...
            <tr>
              ...
              <@flexArea targetId="ClavePresupuestal${item.orderItemSeqId}" title=uiLabelMap.ClavePresupuestal controlStyle="font-weight: bold;" >
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                  <#if tagTypes?has_content>
                    <@cvePresupSelectRows tags=tagTypes entity=item suffix="_${item.orderItemSeqId}"/>
                  </#if>
                </table>
                </@flexArea>
              </td>
