...
<div class="tabletext" style="margin-bottom:250px;">
    <#if reportsGroupedList?has_content>
        <#list reportsGroupedList as group>
            <p><b>${uiLabelMap.get(group.description)}</b>
            <#assign reports = group.reports/>
            <ul class="bulletList">
            <#list reports?default([]) as report>
                <li>
                  <a href="<@ofbizUrl>setupReport?reportId=${report.reportId}</@ofbizUrl>">${uiLabelMap.get(report.shortName)}</a><#if report.description?has_content>: ${uiLabelMap.get(report.description)}</#if>                
                </li>
            </#list>
            </ul>
            </p>
