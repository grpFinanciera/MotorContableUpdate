    ...
    <table class="listTable">
        ...
        <tr class="${tableRowClass(supplier_index)}">
            <#if parameters.sectionName?default("") == "lookup">
                <@displayLinkCell href="javascript:set_values('${supplier.partyId}','${supplier.compositeName?if_exists}')" text="${supplier.compositeName?if_exists} (${supplier.partyId})" blockClass="fieldWidth25pct"/>
            <#else>
