...
import org.opentaps.base.entities.DocumentoObligatorio;
...
public final class OrderServices {
    ...
    public static Map appendOrderItemBasic(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            BigDecimal amount = (BigDecimal) context.get("basePrice");
            if (amount != null) {
            ...
            Map tags = UtilAccountingTags.getTagParameters(context);
              try {
	                //Guarda datos de clave
	                String clavePres = almacenaClavePresupuestal(context, orderItem, delegator, UtilClavePresupuestal.EGRESO_TAG, organizationPartyId,ciclo);
	                //Valida existencia de clave presupuestal
	                UtilClavePresupuestal.existeClavePresu(clavePres.toString(), UtilClavePresupuestal.EGRESO_TAG, delegator);
              } catch (GenericEntityException e) {
              	return UtilMessage.createAndLogServiceError(e, MODULE);
              }
    ...
    public static Map addItemToApprovedOrder(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            Map tags = UtilAccountingTags.getTagParameters(context);
              try {
	                //Guarda datos de clave
	                almacenaClavePresupuestal(context, item, delegator, UtilClavePresupuestal.EGRESO_TAG, organizationPartyId,ciclo);
              } catch (GenericEntityException e) {
              	return UtilMessage.createAndLogServiceError(e, MODULE);
              }
    ...
    public static Map updatePurchaseOrderItems(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            for (OrderItem orderItem : order.getItems()) {
              ...
              //Se obtiene la clave presupuestal
              //Metemos en un mapa las clasificaciones
                Map<String,Object> mapaClas = FastMap.newInstance();
                String clasificacion = new String();
                for (int i = 0; i < tagsMaps.length; i++) {
                	clasificacion = (String)tagsMaps[i].get(orderItem.getOrderItemSeqId());
                	if(clasificacion!=null){
            			mapaClas.put(UtilClavePresupuestal.TAG_PARAM_PREFIX+(i+1), clasificacion);
                	}
        		}

                try {
	                //Guarda datos de clave
	                String clavePres = almacenaClavePresupuestal(mapaClas, orderItem, delegator, UtilClavePresupuestal.EGRESO_TAG, organizationPartyId,ciclo);
                } catch (GenericEntityException e) {
                	return UtilMessage.createAndLogServiceError(e, MODULE);
                }
...
//                }
    ...
    /**
     * Almacena la clave presupuestal en los campos acctgTagEnumId en la orden ingresada 
     ...
     */
	public static String almacenaClavePresupuestal(Map context,OrderItem orderItem,
			Delegator delegator, String acctgTagUsageTypeId, String organizationPartyId, String ciclo) throws GenericEntityException {
		
		int indice = UtilClavePresupuestal.indiceClasAdmin(acctgTagUsageTypeId, organizationPartyId, delegator, ciclo);
		GenericValue clavePresu = UtilClavePresupuestal.obtenEstructPresupuestal(acctgTagUsageTypeId, organizationPartyId, delegator, ciclo);
		
		StringBuffer clavePresupuestal = new StringBuffer(); 
		
		for (int i = 1; i <= UtilClavePresupuestal.TAG_COUNT; i++) {
			String clasificacion = (String) context.get(UtilClavePresupuestal.TAG_PARAM_PREFIX+i);
			if(clasificacion != null)
				clavePresupuestal.append(clasificacion);
			if(indice == i)
				orderItem.set(UtilClavePresupuestal.ENTITY_TAG_PREFIX+"Admin", clasificacion);
			else
				orderItem.set(UtilClavePresupuestal.ENTITY_TAG_PREFIX+i, 
						UtilClavePresupuestal.obtenEnumId(clasificacion, clavePresu.getString("clasificacion"+i), delegator));
     ...
     * @throws GenericEntityException
     */
	public static String almacenaClavePresupuestal(Map context,ShoppingCartItem item,
			Delegator delegator, String acctgTagUsageTypeId, String organizationPartyId, String ciclo) throws GenericEntityException {
		...
		for (int i = 1; i <= UtilClavePresupuestal.TAG_COUNT; i++) {
			...
			if(clasificacion != null)
				clavePresupuestal.append(clasificacion);
			if(indice == i)
				item.setAttribute(UtilClavePresupuestal.ENTITY_TAG_PREFIX+"Admin", clasificacion);
			else
				item.setAttribute(UtilClavePresupuestal.ENTITY_TAG_PREFIX+i, 
						UtilClavePresupuestal.obtenEnumId(clasificacion, clavePresu.getString("clasificacion"+i), delegator));
    ...
    /**
     * Metodo que consulta los pedidos o contratos
     * @param context
     ...
     * @throws GenericEntityException
     */
	public static void buscarPedidoContrato(Map<String, Object> context) throws RepositoryException, ParseException, ListBuilderException, GenericEntityException {
    	Debug.log("Entra a buscar pedidos");
		...
		search.add(EntityCondition.makeCondition(
				StatusItem.Fields.statusTypeId.name(), EntityOperator.EQUALS, "ORDER_STATUS"));
		Debug.log("Mike stattusItem: "+search);
		List<StatusItem> estatus = ledgerRepository.findList(StatusItem.class,
				search);
		List<Map<String, Object>> estatusList = new FastList<Map<String, Object>>();
		for (StatusItem s : estatus) {
			Map<String, Object> map = s.toMap();
			estatusList.add(map);
		}
		Debug.log("Mike estatusList: "+estatusList);
		ac.put("estatusList", estatusList);

		// possible fields we're searching by
		final String orderId = ac.getParameter("orderId");
		String orderName = ac.getParameter("orderName");
		String supplierPartyId = ac.getParameter("supplierPartyId");
		...
		if ("Y".equals(ac.getParameter("performFind"))) {
			...
			if (UtilValidate.isNotEmpty(organizationPartyId) && organizationPartyId != null) {
	            searchConditions.add(EntityCondition.makeCondition("billToPartyId", EntityOperator.EQUALS, organizationPartyId));  
	        }
			...
			if (UtilValidate.isNotEmpty(orderName)) {
	            searchConditions.add(EntityCondition.makeCondition("orderName", EntityOperator.LIKE, "%"+orderName+"%"));  
	        }
			if (UtilValidate.isNotEmpty(supplierPartyId)) {
	            searchConditions.add(EntityCondition.makeCondition("billFromPartyId", EntityOperator.EQUALS, supplierPartyId));  
	        }
			...
			if (UtilValidate.isNotEmpty(fechaContableDesde)) {
	        	searchConditions.add(EntityCondition.makeCondition("entryDate", EntityOperator.GREATER_THAN_EQUAL_TO, UtilDateTime.getDayStart(UtilDateTime.stringToTimeStamp(fechaContableDesde, dateFormat, timeZone, locale), timeZone, locale)));
	        }
	        if (UtilValidate.isNotEmpty(fechaContableHasta)) {
	        	searchConditions.add(EntityCondition.makeCondition("entryDate", EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayStart(UtilDateTime.stringToTimeStamp(fechaContableHasta, dateFormat, timeZone, locale), timeZone, locale)));
	        }
	        if (UtilValidate.isNotEmpty(requisicionId)) {
	            searchConditions.add(EntityCondition.makeCondition("requisicionId", EntityOperator.EQUALS, requisicionId));  
	        }
	        if (UtilValidate.isNotEmpty(userLoginId)) {
	            searchConditions.add(EntityCondition.makeCondition("createdBy", EntityOperator.EQUALS, userLoginId));  
	        }
	        ...
	        List<String> fieldsToSelect = UtilMisc.toList("orderId", "orderName", "entryDate", "statusId", "grandTotal",
	        		"billFromPartyId", "billToPartyId", "createdStamp", "description", "currencyUom", "createdBy");
	        List<String> orderBy = UtilMisc.toList("orderId");
	        
	        List<GenericValue> listPedido = delegator.findByCondition("BuscarPedidos", condiciones, fieldsToSelect, orderBy);

			List<Map<String,Object>> orderListBuilder = FastList.newInstance();
			
			Map<String,Object> mapaPedido = FastMap.newInstance();
			
			for (GenericValue Pedido : listPedido) {
				mapaPedido.putAll(Pedido.getAllFields());
				mapaPedido.put("orderId", Long.valueOf(Pedido.getString("orderId")));
				orderListBuilder.add(mapaPedido);
				mapaPedido = FastMap.newInstance();
			}
			
			Comparator<Map<String, Object>> mapComparator = new Comparator<Map<String, Object>>() {
				public int compare(Map<String, Object> m1, Map<String, Object> m2) {
					return ((Long) m1.get("orderId")).compareTo((Long) m2.get("orderId"));
				}
			};

			Collections.sort(orderListBuilder, mapComparator);
	        
	        ac.put("orderListBuilder", orderListBuilder);
	        Debug.logInfo("orderListBuilder .- "+ orderListBuilder,MODULE);
        }
		
    }
    
    public static String obtenerNombre(Delegator delegator, String partyId) throws GenericEntityException{
    	String nombre = null;
    	
    	GenericValue party = delegator.findByPrimaryKey("Party",
                UtilMisc.toMap("partyId", partyId));
    	GenericValue partyType = delegator.findByPrimaryKey("PartyType",
                UtilMisc.toMap("partyTypeId", party.getString("partyTypeId")));
    	GenericValue partyGroup = delegator.findByPrimaryKey("PartyGroup",
                UtilMisc.toMap("partyId", partyId));
    	
    	if(UtilValidate.isNotEmpty(partyGroup)){
    		nombre = partyGroup.getString("groupName");
    	}else{
    		
    		StringBuffer nombreAuxiliar = new StringBuffer();
        	//Si no se encuentra como grupo se busca como persona
        	if(UtilValidate.isEmpty(partyGroup)){
            	GenericValue partyPerson = delegator.findByPrimaryKey("Person",
                        UtilMisc.toMap("partyId", partyId));
            	if(UtilValidate.isNotEmpty(partyPerson)){
            		nombreAuxiliar.append(partyPerson.getString("firstName") == null ? "" : partyPerson.getString("firstName")+" ");
            		nombreAuxiliar.append(partyPerson.getString("middleName") == null ? "" : partyPerson.getString("middleName")+" ");
            		nombreAuxiliar.append(partyPerson.getString("lastName") == null ? "" : partyPerson.getString("lastName"));
            	}
        	} else {
        		nombreAuxiliar.append(partyGroup.getString("groupName"));
        	}

        	nombre = nombreAuxiliar.toString();
     ...
     * @param context
     */
    public static void documentosPendientes(Map<String, Object> context) {
		...
		try {
			...
			List<DocumentoObligatorio> documentosObligatorios = null;
			List<EntityCondition> searchConditions = new FastList<EntityCondition>();
			...
			if(orderId != null){
				...
				searchConditions.add(EntityCondition.makeCondition(DocumentoObligatorio.Fields.orderId.name(),EntityOperator.EQUALS, orderId));
		
				documentosObligatorios = ledgerRepository.findList(DocumentoObligatorio.class, searchConditions, UtilMisc.toList("docId"));
				
				if(documentosObligatorios.isEmpty()){
					EntityCondition condicion1 = EntityCondition.makeCondition(EntityOperator.AND, 
	    					EntityCondition.makeCondition("tipoAdjudicacionId",	EntityOperator.EQUALS, order.getString("tipoAdjudicacionId")));

	    			List<GenericValue> busqueda = delegator.findByCondition("TipoAdjudicacionDoc", condicion1,	null, UtilMisc.toList("docTipoAdjudicacionId ASC"));
	    			
	    			if(!busqueda.isEmpty()){
	    				for (GenericValue doc : busqueda) {
	    					if(doc.getString("flagArticulo") == null){
	    						GenericValue guardarDoc = GenericValue.create(delegator
		    							.getModelEntity("DocumentoObligatorio"));
		    					guardarDoc.set("docId", delegator.getNextSeqId("DocumentoObligatorio"));
		    					guardarDoc.set("orderId", orderId);
		    					guardarDoc.set("tipoAdjudicacionId", doc.getString("tipoAdjudicacionId"));
		    					guardarDoc.set("docTipoAdjudicacionId", doc.getString("docTipoAdjudicacionId"));
		    					guardarDoc.set("flag", "N");
		    					//userC
		    					delegator.create(guardarDoc);
	    					}
	    				}
	    			}	
				} else if(ac.getParameter("flagV") != null){
					
					for (DocumentoObligatorio s : documentosObligatorios) {
						Map<String, Object> map = s.toMap();
						String adj = (String) map.get("docTipoAdjudicacionId");
	
						GenericValue guardarDoc = delegator.findByPrimaryKey("DocumentoObligatorio",
								UtilMisc.toMap("docId", map.get("docId")));

    					//guardarDoc.set("docId", map.get("docId"));
    					if(ac.getParameter(adj) != null){
    						guardarDoc.set("flag", "Y");
    						guardarDoc.set("userM", userLogin.getString("userLoginId"));
        					guardarDoc.set("fechaActualizacion", UtilDateTime.nowTimestamp());
    					}else{
    						guardarDoc.set("flag", "N");
    					}
    					
    					if(guardarDoc.getString("userC") == null && ac.getParameter(adj) != null){
    						guardarDoc.set("userC", userLogin.getString("userLoginId"));
    					}
    					
    					delegator.store(guardarDoc);
			...
			for (DocumentoObligatorio s : documentosObligatorios) {
				...
				GenericValue docs = delegator.findByPrimaryKey("TipoAdjudicacionDoc",
						UtilMisc.toMap("tipoAdjudicacionId", map.get("tipoAdjudicacionId"),
								"docTipoAdjudicacionId", map.get("docTipoAdjudicacionId")));
				if(UtilValidate.isNotEmpty(docs.getString("notaDocumento")) && docs.getString("notaDocumento").equals(".")){
					map.put("descripcion", docs.getString("nombreDocumento"));
				}else{
					map.put("descripcion", docs.getString("nombreDocumento") + " " + docs.getString("notaDocumento"));
				}
				
				String userC = (String) map.get("userC");
				String userM = (String) map.get("userM");
				
				if(userC != null){
					GenericValue userCr = delegator.findByPrimaryKey("UserLogin",
							UtilMisc.toMap("userLoginId", userC));
					GenericValue personC = delegator.findByPrimaryKey("Person",
							UtilMisc.toMap("partyId", userCr.getString("partyId")));
					map.put("userCr", personC.getString("firstName")+" "+personC.getString("lastName"));
				}
				
				if(userM != null){
					GenericValue userMo = delegator.findByPrimaryKey("UserLogin",
							UtilMisc.toMap("userLoginId", userM));
					GenericValue personM = delegator.findByPrimaryKey("Person",
							UtilMisc.toMap("partyId", userMo.getString("partyId")));
					map.put("userMo", personM.getString("firstName")+" "+personM.getString("lastName"));
				}
			...
			List<Map<String, Object>> ohL = new FastList<Map<String, Object>>();
			
			if(orderId != null){

				List<OrderHeader> oH = ledgerRepository.findList(OrderHeader.class, searchConditions, null);

				for (OrderHeader s : oH) {
					Map<String, Object> map = s.toMap();
					
					String bill = (String) map.get("billFromPartyId");
					GenericValue pa = delegator.findByPrimaryKey("PartyGroup",UtilMisc.toMap("partyId", bill));
					
					if(pa != null){
						map.put("groupName", pa.getString("groupName"));
					}else{
						map.put("groupName", "NA");
					}

					GenericValue ad = delegator.findByPrimaryKey("TipoAdjudicacion",
							UtilMisc.toMap("tipoAdjudicacionId", map.get("tipoAdjudicacionId")));
					
					if(UtilValidate.isNotEmpty(ad)){
						map.put("nombreAd", ad.getString("nombreTipoAdjudicacion"));
					} else {
						map.put("nombreAd", "");
					}
			...
			ac.put("documentosList", documentosList);
    ...
    /**
     * Metodo que se utiliza para actualizar la fecha de entrega de cada articulo de una requisicion
     * @param context
     */
    public static void cambioFechasProducto(Map<String, Object> context) {
		...
		try {
			...
			List<EntityCondition> searchConditions = new FastList<EntityCondition>();
			List<Map<String, Object>> orderItem = new FastList<Map<String, Object>>();
	
			if(UtilValidate.isNotEmpty(orderId)){

				listOrderItem = delegator.findByCondition("OrderItem", EntityCondition.makeCondition("orderId", orderId), null, UtilMisc.toList("orderItemSeqId"));
				
				for (GenericValue OrderItem : listOrderItem) {
					Map<String, Object> mapOrderItem = OrderItem.getAllFields();
					String orderItemSeqId = (String) mapOrderItem.get("orderItemSeqId");
					GenericValue DetalleRequisicion = (GenericValue) OrderItem.getRelatedOne("DetalleRequisicion");
					
					if(UtilValidate.isNotEmpty(DetalleRequisicion) && UtilValidate.isNotEmpty(ac.getParameter(orderItemSeqId))){
						Date fechaEntrega = UtilDateTime.getDateFromString(UtilDateTime.getDateFormat(locale), timeZone, locale, ac.getParameter(orderItemSeqId));
						DetalleRequisicion.set("fechaEntrega",  UtilDateTime.toTimestamp(fechaEntrega));
						DetalleRequisicion.store();
					}
					
					if(UtilValidate.isNotEmpty(DetalleRequisicion)){
						mapOrderItem.put("fechaEntrega", DetalleRequisicion.getTimestamp("fechaEntrega"));
					}
					
					orderItem.add(mapOrderItem);
				}
				
				ac.put("orderItem", orderItem);
				ac.put("orderId", orderId);
	...
	/**
	 * Consulta las penas aplicadas 
	 * @param context
	 * @throws GenericEntityException
	 */
	public static void consultarPenalizacion(Map<String, Object> context) throws GenericEntityException {
		...
		try {
			...
			String aplicaPenaId = ac.getParameter("aplicaPenaId");
			String performFind = ac.getParameter("performFind");
			List<EntityCondition> searchConditions = new FastList<EntityCondition>();
			searchConditions.add(EntityCondition.makeCondition("penaDeductivaFlag",EntityOperator.EQUALS, "P"));
			
			if(UtilValidate.isNotEmpty(performFind) && performFind.trim().equals("Y")){
				
				if (UtilValidate.isNotEmpty(penaId)) {
					searchConditions.add(EntityCondition.makeCondition("aplicaPenaId",EntityOperator.EQUALS, penaId));
				}
				
				if (UtilValidate.isNotEmpty(ordenCobroId)) {
					searchConditions.add(EntityCondition.makeCondition("invoiceId",EntityOperator.EQUALS, ordenCobroId));
				}
				
				if(UtilValidate.isNotEmpty(aplicaPenaId)){
					searchConditions.add(EntityCondition.makeCondition("aplicaPenaId",EntityOperator.EQUALS, aplicaPenaId));
				}
				
				List<Map<String, Object>> penasAplicadasList = new FastList<Map<String, Object>>();
				List<GenericValue> listDetallePenas = delegator.findByCondition("BuscarDetallePenas", EntityCondition.makeCondition(searchConditions), null, null);
				
				for (GenericValue DetallePenas : listDetallePenas) {
					penasAplicadasList.add(DetallePenas.getAllFields());
				}
				
				ac.put("penasAplicadasList", penasAplicadasList);
				ac.put("aplicaPenaId", aplicaPenaId);
				ac.put("invoiceId", ordenCobroId);
				ac.put("penaId", penaId);
