...
public final class OrderEvents {
     ...
     */
    public static String destroyCart(HttpServletRequest request, HttpServletResponse response, String applicationName) throws GenericEntityException {
    ...
    /**
     * Agrega varios articulos a la orden de compra
     * @param request
     * @param response
     * @return
     * @throws GenericEntityException 
     */
    public static String agregaArticulosOrden(HttpServletRequest request, HttpServletResponse response){
        ...
        if (UtilValidate.isEmpty(cart)) {
            Debug.logWarning("No se encontro el pedido", MODULE);
            UtilMessage.addInternalError(request);
            return "error";
        }
        com.ibm.icu.util.Calendar calFechaOrden = UtilDateTime.getCalendarInstance(timeZone, locale);
        String mesIdActual = UtilFormatOut.formatPaddedNumber(calFechaOrden.get(Calendar.MONTH)+1, 2);
        String momentoCompara = new String();
        
        try {
        	
			GenericValue LineaPresupuestal = EntityUtil.getFirst(delegator.findByAnd("LineaPresupuestal",UtilMisc.toMap("acctgTransTypeId",cart.getAcctgTransTypeId())));
			momentoCompara = LineaPresupuestal.getString("momentoCompara");

	        LinkedHashSet<String> listClaves = new LinkedHashSet<String>();
	        List<ShoppingCartItem> listShoppingCartItem = FastList.newInstance();
	        
	        StringBuffer errorClave = new StringBuffer();
	        
	        String[] mapaCheck = request.getParameterValues("check");
	        
	        if(UtilValidate.isNotEmpty(mapaCheck)){
	            //productoId:_cantidad:_monto:_detalleRequisicionId:_requisicionId:_customTimePeriodId:_clasificacion1:_clasificacion2...:_clasificacion15
	        	
	            for (String cadenaArticulo : mapaCheck) {
	            	String productId = "";
	            	
	                	int elemento = 0;
	                	
	    	        	String[] articulo = cadenaArticulo.split(":_");
	    	        	productId = articulo[elemento++];
	    	        	Double cantidad = Double.parseDouble(articulo[elemento++]);
	    	        	Double monto = Double.parseDouble(articulo[elemento++]);
	    	        	String detalleRequisicionId = articulo[elemento++];
	    	        	String requisicionId = articulo[elemento++];
	    	        	String customTimePeriodId = articulo[elemento++];
	    	        	String mesId = articulo[elemento++];
	    	        	String descripcionDetalle = null;
	    	        	Timestamp fechaEntrega = null;
	    	        	
	    	        	if(detalleRequisicionId != null && requisicionId != null){
	    	        		GenericValue DetalleRequisicion = delegator.findByPrimaryKeyCache("DetalleRequisicion", 
	    	        				UtilMisc.toMap("detalleRequisicionId",detalleRequisicionId, "requisicionId",requisicionId));
	    	        		
	    	        		descripcionDetalle = DetalleRequisicion.getString("descripcion");
	    	        		fechaEntrega = DetalleRequisicion.getTimestamp("fechaEntrega");
	    	        		if(UtilValidate.isEmpty(customTimePeriodId)){
	        	        		customTimePeriodId = DetalleRequisicion.getString("customTimePeriodId");
	        	        		if(UtilValidate.isEmpty(customTimePeriodId)){
	        	        			mesId = mesIdActual;
	        	        		} else {
	        	        			Calendar calFromDate = UtilDate.toCalendar(DetalleRequisicion.getRelatedOne("CustomTimePeriod").getDate("fromDate"), timeZone, locale);
	        	        			mesId = UtilFormatOut.formatPaddedNumber(calFromDate.get(Calendar.MONTH)+1,2);
	        	        		}
	    	        		}
	    	        	}
	    	            ...
	    	            for (int i = elemento ; i < articulo.length; i++) {
	    	            	if(UtilValidate.isNotEmpty(articulo[i])){
	    	            		attributes.put("tag"+(i-6), articulo[i]);
	    	            		clavePresupuestal.append(articulo[i]);
	    	            	}
	    	            }
	    	            UtilClavePresupuestal.existeClavePresu(clavePresupuestal.toString(), UtilClavePresupuestal.EGRESO_TAG, delegator);
	    	            
	    	            listClaves.add(clavePresupuestal.toString());
	    	            ...
	    	            listShoppingCartItem.add(ShoppingCartItem.makePurchaseOrderItem(null, productId, BigDecimal.valueOf(monto), BigDecimal.valueOf(cantidad), 
	    	            		null, attributes, null, null,
	    	            		null, null, dispatcher, cart, null, null, null, null,
	    	            		detalleRequisicionId, requisicionId, descripcionDetalle, customTimePeriodId, mesId, fechaEntrega));
	    	            
	            	}
	            
	            Map<String,Object> mapaClaveMesMonto = FastMap.newInstance();
	            
	    		List <EntityCondition> condicionesControl = new FastList<EntityCondition>(); 
	    		condicionesControl.add(EntityCondition.makeCondition(EntityOperator.AND,
	    				EntityCondition.makeCondition("momentoId", momentoCompara),
	    				EntityCondition.makeCondition("clavePresupuestal", EntityOperator.IN, listClaves)));
	            
	            List<GenericValue> listControlPresupuestal = delegator.findByAnd("ControlPresupuestal", condicionesControl);
	            
	            for (GenericValue ControlPresupuestal : listControlPresupuestal) {
	            	String llave = ControlPresupuestal.getString("clavePresupuestal")+
	            					ControlPresupuestal.getString("mesId");
	            	UtilMisc.addToBigDecimalInMap(mapaClaveMesMonto, llave, ControlPresupuestal.getBigDecimal("monto"));
				}
	            
	            //Validacion de presupuesto
	            for (ShoppingCartItem ShoppingCartItem : listShoppingCartItem) {
	            	BigDecimal montoCompara = UtilNumber.getBigDecimal(ShoppingCartItem.getSelectedAmount().multiply(ShoppingCartItem.getQuantity()));
	            	
	            	Map mapaClave = ShoppingCartItem.getAttributes();
    	            StringBuffer clavePresupuestal = new StringBuffer();
    	            for (int i = 1 ; i <= UtilClavePresupuestal.TAG_COUNT; i++) {
    	            	String clasificacion = (String) mapaClave.get("tag"+i);
    	            ...
    	            String mesId = ShoppingCartItem.getMesId();
    	            String llave = clavePresupuestal+mesId;
    	            BigDecimal montoControl = UtilNumber.getBigDecimal((BigDecimal) mapaClaveMesMonto.get(llave));
    	            if(montoCompara.compareTo(montoControl) > 0){
    	            	errorClave.append("La clave presupuestal ["+clavePresupuestal+"] con cuenta con suficiencia : "+
    	            						montoCompara.subtract(montoControl)+" en el mes ["+mesId+"]\n");
    	            	if(UtilValidate.isNotEmpty(mapaClaveMesMonto.get(llave))){
    	            		UtilMisc.addToBigDecimalInMap(mapaClaveMesMonto, llave, montoControl.negate());
    	            	}
    	            }
				}
	            
	            if(UtilValidate.isEmpty(errorClave)){
	            	ServiceUtil.setMessages(request, null, "Se agregaron el o los art\u00edculos correctamente", null);
	            } else {
	            	for (ShoppingCartItem ShoppingCartItem : listShoppingCartItem) {
	            		cart.removeCartItem(ShoppingCartItem, dispatcher);
	            	}
		            UtilMessage.addError(request, errorClave.toString());
		            return "error";
	            }
	
	        } else {
	            UtilMessage.addError(request, "OpentapsError_CannotAddItem", UtilMisc.toMap("message", "No hay art"));
	            return "error";
	        }
    ...
    public static String addOrderItem(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
    	Debug.log("Se ingresa a crear item para la orden: " + request);
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        ...
        try {
            Map<String,Object> results = dispatcher.runSync("getProductByComprehensiveSearch", UtilMisc.toMap("productId", productId));
        ...
        Debug.log("if it's a variant product, send to virtualProduct request ");
        ...
        Debug.log("still no product, send to lookup request ");
        if (product == null) {
            return "findMatchingProducts";
        }

        // if the product is configurable or virtual, send user to appropriate page
        Debug.log("if the product is configurable or virtual, send user to appropriate page ");
        ...
        // validate quantity
        Debug.log("validate quantity ");
        double quantity = 0;
        Double monto = null;
        String detalleRequisicionId = UtilCommon.getParameter(request, "detalleRequisicionId");
        String requisicionId = UtilCommon.getParameter(request, "requisicionId");
        String quantityString = UtilCommon.getParameter(request, "quantity");
        String montoString = UtilCommon.getParameter(request, "monto");
        String customTimePeriodId = UtilCommon.getParameter(request, "customTimePeriodId");
        String cantidadMeses = UtilCommon.getParameter(request, "cantidadMeses");
        String descripcionDetalle = null;
        Timestamp fechaEntrega = null;
        
        int meses = Integer.valueOf(cantidadMeses);
		int customTime = Integer.valueOf(customTimePeriodId);
		for (int j=1; j<=meses; j++){
        
        if(detalleRequisicionId != null && requisicionId != null){
    		GenericValue detalle = delegator.findByPrimaryKey("DetalleRequisicion", 
    				UtilMisc.toMap("detalleRequisicionId",detalleRequisicionId, "requisicionId",requisicionId));
    		descripcionDetalle = detalle.getString("descripcion");
        ...
        try {
            ...
            if(montoString != null){
            	monto = Double.valueOf(montoString);
            }
        ...
        UtilAccountingTags.addTagParameters(request, tags,"clasificacion");
        UtilAccountingTags.addTagParameters(tags, attributes);
        StringBuffer clavePresupuestal = new StringBuffer();
        for (int i = 1; i <= 15; i++) {
        	String clasN = UtilCommon.getParameter(request, "clasificacion"+i);
        	Debug.log("clasificacion"+ i + ": " + clasN);
        	clavePresupuestal = clavePresupuestal.append(clasN == null?"":clasN);
		}

        //Valida que exista la clave presupuestal
		GenericValue cvePrespu = delegator.findByPrimaryKey("ClavePresupuestal",
				UtilMisc.toMap("clavePresupuestal", clavePresupuestal.toString()));
		if (UtilValidate.isEmpty(cvePrespu)){
			UtilMessage.addError(request, "No existe la clave presupuestal", UtilMisc.toMap("message","No existe la clave presupuestal"));
			return "error";
		}
		
		/**
		 * Validación presupuestal de la nueva clave
		 */
        GenericValue LineaPresupuestal = EntityUtil.getFirst(delegator.findByCondition("LineaPresupuestal", EntityCondition.makeCondition("acctgTransTypeId", cart.getAcctgTransTypeId()), null, UtilMisc.toList("secuencia")));
        String momentoCompara = LineaPresupuestal.getString("momentoCompara");
        
        if(UtilValidate.isEmpty(customTimePeriodId)){
            UtilMessage.addRequiredFieldError(request, "mes");
            return "error";
        }
        
        GenericValue CustomTimePeriod = delegator.findByPrimaryKey("CustomTimePeriod",UtilMisc.toMap("customTimePeriodId",customTimePeriodId));
        
        Date fechaInical = CustomTimePeriod.getDate("fromDate");
        com.ibm.icu.util.Calendar calendarioFecha = UtilDateTime.toCalendar(fechaInical, timeZone, locale);
        String mesId = UtilFormatOut.formatPaddedNumber(calendarioFecha.get(Calendar.MONTH)+1,2);
		
		List<EntityExpr> condicionesPresupuesto = UtilMisc.toList(EntityCondition.makeCondition("clavePresupuestal", clavePresupuestal.toString()),
				EntityCondition.makeCondition("momentoId", momentoCompara),
                EntityCondition.makeCondition("ciclo", String.valueOf(calendarioFecha.get(Calendar.YEAR))),
                EntityCondition.makeCondition("mesId", mesId));
		
		GenericValue ControlPresupuestal = EntityUtil.getFirst(
				delegator.findByCondition("ControlPresupuestal", EntityCondition.makeCondition(condicionesPresupuesto) , null, null));
		
		BigDecimal montoOriginal = BigDecimal.valueOf(quantity*monto);
		BigDecimal montoCompara = BigDecimal.ZERO;
		
		if(UtilValidate.isNotEmpty(ControlPresupuestal)){
			montoCompara = ControlPresupuestal.getBigDecimal("monto");
		} else if (UtilValidate.isEmpty(ControlPresupuestal) || montoOriginal.compareTo(montoCompara) > 0){
			UtilMessage.addError(request, "La clave presupuestal ["+cvePrespu+"] con cuenta con suficiencia");
			return "error";
		}
        ...
        try {
            ...
            addItemToOrder(cart, productId, monto, quantity, null, null, null, shipBeforeDate, null, null, 
            		attributes, productCatalogId, null, itemType, null, null, request, 
            		detalleRequisicionId, requisicionId, descripcionDetalle,customTimePeriodId,mesId,fechaEntrega);
    ...
    public static int addItemToOrder(ShoppingCart cart, String productId, Double selectedAmountDbl, double quantity, Timestamp reservStart, Double reservLengthDbl, Double reservPersonsDbl,
            ...
            ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber, String parentProductId, HttpServletRequest request,
            String detalleRequisicionId, String requisicionId, String descripcionDetalle, String customTimePeriodId, String mesId, Timestamp fechaEntrega) throws CartItemModifyException, ItemNotFoundException, RepositoryException, InfrastructureException, GenericServiceException, GenericEntityException {
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        ...
        if (cart.getOrderType().equals("PURCHASE_ORDER")) {
            ...
            PurchasingRepositoryInterface purchasingRepository = domainLoader.getDomainsDirectory().getPurchasingDomain().getPurchasingRepository();
        ...
        Debug.log("Monto real: " + selectedAmountDbl);
        ...
        Debug.log("Mike cart: " + cart.items());
        int index = cart.addOrIncreaseItem(productId, selectedAmount, quantityBd, reservStart, reservLength, reservPersons,
                ...
                configWrapper, itemType, itemGroupNumber, parentProductId, dispatcher, detalleRequisicionId, requisicionId,
                descripcionDetalle,customTimePeriodId,mesId,fechaEntrega);
        return index;
    }
    ...
    public static ShoppingCart getOrInitializeCart(HttpServletRequest request) {
        ...
        OpentapsShoppingCart opentapsCart = new OpentapsShoppingCart(delegator, productStoreId, webSiteId, UtilHttp.getLocale(request), 
        		currencyUomId, billToCustomerPartyId, billFromVendorPartyId, "", null, null, null, null, null, null, null, null, null, null);
	...
	public static String modifyCart(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException{
    	
    	Debug.log("Entra a modificar item");
    	if ("D".equals(request.getParameter("performFind"))) {
    		Debug.log("solo refresca");
    		return "success";
    	}
        HttpSession session = request.getSession();
    ...
    public static Map prepareOrderReportParameters(DomainsLoader dl, Delegator delegator, LocalDispatcher dispatcher, GenericValue userLogin, Locale locale, String orderId, String organizationPartyId) throws GenericServiceException, GenericEntityException, PartyNotFoundException, RepositoryException, EntityNotFoundException {
        ...
        for (org.opentaps.domain.order.OrderItem orderItem : order.getOrderItems()) {
            ...
            String descripcion = (orderItem.getItemDescription() == null ? 
            								(orderItem.getProduct() == null ? "" : orderItem.getProduct().getProductName()) : orderItem.getItemDescription());
            reportLine.put("itemDescription", descripcion);
            reportLine.put("orderQuantity", orderItem.getOrderedQuantity());
            reportLine.put("orderUnitPrice", orderItem.getSelectedAmount());
        ...
        GenericValue partySup = delegator.findByPrimaryKey("PartyGroup", UtilMisc.toMap("partyId",order.getBillFromVendor().getPartyId()));
