...
public final class CommonEvents {
    ...
    public static String setOrganization(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (UtilValidate.isEmpty(cicloId)) {
            try {
            	cicloId = UtilCommon.getUserLoginViewPreference(request, UtilConfig.SYSTEM_WIDE, UtilConfig.SET_ORGANIZATION_FORM, UtilConfig.OPTION_DEF_CICLO);
            } catch (GenericEntityException e) {
                Debug.logError(e, "Error al obtener el ciclo por defecto", MODULE);
                return "selectOrganization";
            }
        ...
        try {
        	CustomTimePeriod = EntityUtil.getFirst(delegator.findByAndCache("CustomTimePeriod", UtilMisc.toMap("periodName", cicloId, "periodTypeId","FISCAL_YEAR","isClosed","N")));
        } catch (GenericEntityException e) {
            Debug.logError(e, "No se pudo obtener el ciclo o esta cerrado", MODULE);
        }
        ...
        try {
            ...
            UtilCommon.setUserLoginViewPreference(request, UtilConfig.SYSTEM_WIDE, UtilConfig.SET_ORGANIZATION_FORM, UtilConfig.OPTION_DEF_CICLO, cicloId);
        } catch (GenericEntityException e) {
    ...
    public static String printJRReport(HttpServletRequest request, HttpServletResponse response) throws EventHandlerException {
        ...
        String serverRootUrl = UtilHttp.getServerRootUrlForceHttp(request).toString();
        parameters.put("serverRootUrl", UtilValidate.isEmpty(serverRootUrl) ? "" : serverRootUrl );
       ...
       //UtilMessage.createServiceSuccess("Report was sent to printer [" + printerName + "] successfuly.", locale,parameters);
        ServiceUtil.setMessages(request, null, "Report was sent to printer [" + printerName + "] successfuly.", null);
        return "success";
    }
