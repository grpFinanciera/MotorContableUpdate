...
public final class InvoiceHelper {
    ...
    public static Map<String, Object> joinInvoiceItemForPresentation(GenericValue invoiceItem) throws GenericEntityException {
        ...
        //Se busca , si existe la descripcion de la requisicion , de otro modo se deja la de invoice
        List<GenericValue> listDescripciones = invoiceItem.getDelegator().findByAnd("DescripcionArticulo",
        		UtilMisc.toMap("invoiceItemSeqId",invoiceItem.getString("invoiceItemSeqId"),"invoiceId",invoiceItem.getString("invoiceId")));
        if(UtilValidate.isNotEmpty(listDescripciones)){
        	if(UtilValidate.isNotEmpty(listDescripciones.get(0).getString("descripcionOrd"))){
        		description = listDescripciones.get(0).getString("descripcionOrd");
        	}
        }
     ...
     * @exception GenericEntityException if an error occurs
     */
    public static List<Map<String, Object>> getInvoiceLinesForPresentation(Delegator delegator, String invoiceId, Boolean groupSalesTaxOnInvoicePdf, String tipoTransac,String organizationPartyId,String ciclo) throws GenericEntityException {
        ...
        for (GenericValue orderItem : orderItemGroup.keySet()) {
            ...
            String area = UtilClavePresupuestal.buscaExternalId(orderItem.getString(UtilClavePresupuestal.ENTITY_TAG_PREFIX+"Admin"), delegator);
            String clavePresupuestal = UtilClavePresupuestal.getClavePresupuestal(orderItem, delegator, tipoTransac, organizationPartyId,ciclo);
            
            invoiceLine.put("area", area);
            invoiceLine.put("clavePresupuestal", clavePresupuestal);
