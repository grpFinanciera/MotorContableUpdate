...
public final class UtilAccountingTags {
    ...
    public static final String ENTITY_VIEW_PREFIX = "clasificacion";
    ...
    public static Map<Integer, String> getAccountingTagTypesForOrganization(String organizationPartyId, String accountingTagUsageTypeId, Delegator delegator) throws RepositoryException {
        ...
        repository.setDelegator(delegator);
    ...
    public static void putAllAccountingTags(GenericValue value, Map map, String mapPrefix) {
        for (int i = 1; i <= TAG_COUNT; i++) {
            ...
            if(value.get(ENTITY_TAG_PREFIX +"Admin") != null)
            	map.put(mapPrefix+"Admin", value.get(ENTITY_TAG_PREFIX+"Admin"));
        }
    }
    ...
    public static List<GenericValue> getBancosSucursales(Delegator delegator) throws RepositoryException {
    	
    	

        List<GenericValue> Banco = null;
        List<String> orderBy = UtilMisc.toList("nombreBanco");
		try {
			Banco = delegator.findAll("Banco", orderBy);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		Debug.log("Banco: " + Banco);
	...
	public static List<ClassificationConfigurationForOrganization> getClassificationTagsForOrganization(String organizationPartyId,String cicloId, String accountingTagUsageTypeId, Delegator delegator) throws RepositoryException {
        OrganizationRepository repository = new OrganizationRepository();
        repository.setDelegator(delegator);
        return repository.getClassificationTagConfiguration(organizationPartyId, cicloId, accountingTagUsageTypeId);
    }
