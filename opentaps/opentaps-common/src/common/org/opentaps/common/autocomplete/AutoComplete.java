...
import static org.opentaps.common.autocomplete.UtilAutoComplete.AC_ACCOUNT_FIELDS_REGISTRO;
...
public final class AutoComplete {
    ...
    public static String getAutoCompleteSupplierPartyIds(HttpServletRequest request, HttpServletResponse response) {
        ...
        EntityCondition condicionORSupplier = EntityCondition.makeCondition(EntityOperator.OR,
        		EntityCondition.makeCondition("roleTypeId", "SUPPLIER"),
        		EntityCondition.makeCondition("roleTypeId", "PROVEEDOR")
            );
        EntityCondition condition = EntityCondition.makeCondition(EntityOperator.AND,
                                   ac_activePartyCondition,
                                   condicionORSupplier);
    ...
    private static String autocompletePartyIdsByCondition(EntityCondition condition, String entityName, HttpServletRequest request, HttpServletResponse response) {
        ...
        EntityCondition condicionOR = EntityCondition.makeCondition(EntityOperator.OR,
        		EntityCondition.makeCondition("partyId", EntityOperator.LIKE , "%"+keyword+"%"),
        		EntityCondition.makeCondition("groupName", EntityOperator.LIKE , "%"+keyword+"%"),
        		EntityCondition.makeCondition("lastName", EntityOperator.LIKE , "%"+keyword+"%"),
        		EntityCondition.makeCondition("firstName", EntityOperator.LIKE , "%"+keyword+"%")
        		);
        condition = EntityCondition.makeCondition(EntityOperator.AND,condition,condicionOR);
        if(!UtilValidate.isEmpty(filtro)){
        	condition = EntityCondition.makeCondition(EntityOperator.AND,EntityCondition.makeCondition("partyTypeId",EntityOperator.EQUALS,filtro)
        			,condition);
        } 
        ...
        if (keyword.length() > 0) {
        	try {
        		parties = delegator.findByCondition(entityName, condition, AC_PARTY_NAME_FIELDS, AP_PARTY_ORDER_BY);
     ...
     * Retrieves the auto complete GL Account IDs with a given keyword.
     * It will match the keyword against either the GL Account name or the account code.
     * Solo obtiene las cuentas de registro
     * @param request a <code>HttpServletRequest</code> value
     * @param response a <code>HttpServletResponse</code> value
     * @return a <code>String</code> value
     */
    public static String getAutoCompleteGlAccountsRegistro(HttpServletRequest request, HttpServletResponse response) {
        GenericValue userLogin = UtilCommon.getUserLogin(request);
        if (userLogin == null) {
            Debug.logError("Failed to retrieve the login user from the session.", MODULE);
            return "error";
        ...
        if (keyword.length() > 0) try {
            ...
            EntityCondition condition = EntityCondition.makeCondition(EntityOperator.AND,
                    ...
                    EntityCondition.makeCondition("organizationPartyId", EntityOperator.EQUALS, organizationPartyId),
                    EntityUtil.getFilterByDateExpr(),
                    EntityCondition.makeCondition("tipoCuentaId", EntityOperator.EQUALS, "R"));


            // get result as a list iterator (transaction block is to work around a bug in entity engine)
            TransactionUtil.begin();
            EntityListIterator iterator = delegator.findListIteratorByCondition("GlAccountOrganizationAndClass", condition, null, AC_ACCOUNT_FIELDS_REGISTRO, AC_ACCOUNT_ORDER_BY, AC_FIND_OPTIONS);
    ...
    public static class PartySelectionBuilder implements SelectionBuilder {
        public Map<String, Object> buildRow(Object element) {
            ...
            String compositeName = PartyHelper.getPartyName(party);
