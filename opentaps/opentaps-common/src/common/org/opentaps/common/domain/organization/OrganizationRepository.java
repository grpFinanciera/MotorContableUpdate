...
public class OrganizationRepository extends PartyRepository implements OrganizationRepositoryInterface {
	...
	public Map<Integer, String> getClassificationTagTypes(
			String organizationPartyId, String accountingTagUsageTypeId, String cicloId)
			throws RepositoryException {
		Date date = new Date();
		Map<Integer, String> tagTypes = new TreeMap<Integer, String>();
		@SuppressWarnings("deprecation")
		List<EstructuraClave> conf = findList(EstructuraClave.class,map(EstructuraClave.Fields.organizationPartyId,organizationPartyId,
				EstructuraClave.Fields.acctgTagUsageTypeId,accountingTagUsageTypeId, EstructuraClave.Fields.ciclo,cicloId));
		if (conf.isEmpty()) {
			Debug.logInfo(
    ...
    /**
	 * {@inheritDoc}
     * @param accountingTagUsageTypeId2 
	 * 
	 * @throws InfrastructureException
	 */
	public List<ClassificationConfigurationForOrganization> getClassificationTagConfiguration(
			String organizationPartyId, String cicloId, String accountingTagUsageTypeId)
			throws RepositoryException {
		...
		Map<Integer, String> tagTypes = getClassificationTagTypes(organizationPartyId, accountingTagUsageTypeId, cicloId);
		...
		for (Integer index : tagTypes.keySet()) {
			...
			if (!listclas.isEmpty()) {
				for (ClasifPresupuestal clasifPresupuestal : listclas) {
					...
					//Se obtiene List<genericValue> de acuerdo a la clasificacion
					if(clasifPresupuestal.getClasificacionId().equals("CL_ADMINISTRATIVA")){
						tag.setTagValues(UtilClassification.getListaCatalogos("NivelesParty", delegator, clasifPresupuestal.getClasificacionId(),false));
						tag.setActiveTagValues(UtilClassification.getListaCatalogos("NivelesParty", delegator, clasifPresupuestal.getClasificacionId(),true));
					} else {
						tag.setTagValues(UtilClassification.getListaCatalogos("Enumeration", delegator, clasifPresupuestal.getClasificacionId(),false));
						tag.setActiveTagValues(UtilClassification.getListaCatalogos("Enumeration", delegator, clasifPresupuestal.getClasificacionId(),true));
					}
		...
		Debug.logInfo("getClassificationTagConfiguration tagTypesAndValues size"+ tagTypesAndValues.size(),MODULE);
