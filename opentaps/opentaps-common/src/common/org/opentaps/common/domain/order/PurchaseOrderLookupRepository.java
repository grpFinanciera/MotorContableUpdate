...
public class PurchaseOrderLookupRepository extends CommonLookupRepository implements PurchaseOrderLookupRepositoryInterface {
    ...
    public List<OrderViewForListing> findOrders() throws RepositoryException {
        ...
        try {
            ...
            // set the order by
            if (orderBy == null) {
                orderBy = Arrays.asList(OrderHeader.Fields.orderId.asc());
            }
            ...
            try {
                ...
                while (o != null) {
                    ...
                    r.calculateExtraFields(getInfrastructure().getDelegator(), timeZone, locale);
                    currency = Currency.getInstance(r.getCurrencyUom() == null ? "MXN" : r.getCurrencyUom());
                    nf.setCurrency(currency);
                    r.setTotal(nf.format(r.getGrandTotal()));
                    results2.add(r);
                    n++;
