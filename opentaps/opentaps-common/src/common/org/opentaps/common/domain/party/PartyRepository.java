...
public class PartyRepository extends DomainRepository implements PartyRepositoryInterface {
    ...
    public List<PostalAddress> getPostalAddresses(Party party, Party.ContactPurpose purpose) throws RepositoryException {
        ...
        try {
            List<GenericValue> purposes = PartyContactHelper.getContactMechsByPurpose(party.getPartyId(), ContactMechTypeConstants.POSTAL_ADDRESS, purposeId, true, getInfrastructure().getDelegator());
    ...
    public List<TelecomNumber> getPhoneNumbers(Party party, Party.ContactPurpose purpose) throws RepositoryException {
        ...
        try {
            List<GenericValue> purposes = PartyContactHelper.getContactMechsByPurpose(party.getPartyId(), ContactMechTypeConstants.TELECOM_NUMBER, purposeId, true, getInfrastructure().getDelegator());
            List<String> contactList = EntityUtil.getFieldListFromEntityList(purposes, ContactMech.Fields.contactMechId.name(), true);
    ...
    public PostalAddress getSupplierPostalAddress(Party party) throws RepositoryException, EntityNotFoundException {
        try {
            GenericValue supplierAddress = PartyContactHelper.getPostalAddressValueByPurpose(party.getPartyId(), ContactMechPurposeTypeConstants.GENERAL_LOCATION, true, getInfrastructure().getDelegator());
            if (supplierAddress == null) {
                supplierAddress = PartyContactHelper.getPostalAddressValueByPurpose(party.getPartyId(), ContactMechPurposeTypeConstants.BILLING_LOCATION, true, getInfrastructure().getDelegator());
            }
            if (supplierAddress == null) {
                supplierAddress = PartyContactHelper.getPostalAddressValueByPurpose(party.getPartyId(), ContactMechPurposeTypeConstants.PAYMENT_LOCATION, true, getInfrastructure().getDelegator());
            }
            if (supplierAddress != null) {
