  ...
  <form method="GET" action="<@ofbizUrl>editProductDeliveryDate</@ofbizUrl>" name="editProductDeliveryDate">
    ...
    <@searchFlexArea searchBy="product" searchOption=searchOption >
      <table class="twoColumnForm">
        ...
        <tr>
          <td>
            <@inputAutoCompleteProductRow name="productId" title=uiLabelMap.ProductProductId default=""/>
      ...
      <table class="listTable">
        ...
        <#list orderItems as orderItem>
          ...
          <tr class="${tableRowClass(orderItem_index)}">
            ...
            <td align="right">
              <#if datesEditable && orderItem.estimatedReadyDate?exists>
				<#assign fecha = Static["org.ofbiz.base.util.UtilDateTime"].timeStampToString(orderItem.estimatedReadyDate,fmt,timeZone,locale) />
                <@inputDate name="estimatedReadyDate" index=orderItem_index default=fecha />
              <#else >
                <@displayDate date=orderItem.estimatedReadyDate format="DATE_ONLY" />
              </#if >
            </td>
          </tr>
        </#list>
      ...
      <table class="listTable">
        ...
        <#list orderItems as orderItem>
          ...
          <tr class="${tableRowClass(orderItem_index)}">
            ...
            <td>${currentItemStatus.get("description",locale)?default(currentItemStatus.statusId)}</td>
            <td align="right">
            	<#if orderItem.estimatedReadyDate?exists>
            		<#assign fecha1 = Static["org.ofbiz.base.util.UtilDateTime"].timeStampToString(orderItem.estimatedReadyDate,fmt,timeZone,locale) />
            	</#if>	
                <@inputDate name="estimatedReadyDate" index=orderItem_index default=fecha1 />
            </td>
          </tr>
        </#list>
