...
public class SupplierServices {
    ...
    public static Map updateSupplier(DispatchContext dctx, Map context) {
        ...
        Debug.log("contextUpdateSupplier: " + context);
        String supplierPartyId = (String) context.get("partyId");
        String rfc = (String) context.get("federalTaxId");
        String regimenId = (String) context.get("regimenId");
        String tamanioAuxiliarId = (String) context.get("tamanioAuxiliarId");
        String sectorEconomicoId = (String) context.get("sectorEconomicoId");
        String origenCapitalId = (String) context.get("origenCapitalId");        
        ...
        try {
            ...
            input.put("rfc", rfc);
            input.put("regimenId", regimenId);
            input.put("tamanioAuxiliarId", tamanioAuxiliarId);
            input.put("sectorEconomicoId", sectorEconomicoId);
            input.put("origenCapitalId", origenCapitalId);
    ...
    public static Map<String, Object> createSupplier(DispatchContext dctx, Map<String, Object> context) {
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        String supplierPartyId = (String) context.get("partyId");
        String roleTypeId = (String) context.get("roleTypeId");
        Debug.logInfo("contextSupplier: " + context,module);                               
        ...
        try {
         // verify account name is use already
            if (!"Y".equals(forceComplete)) {
                ...
                PartyDomainInterface partyDomain = domainLoader.getDomainsDirectory().getPartyDomain();
                PartyRepositoryInterface repo = partyDomain.getPartyRepository();
                //SE COMENTA LA SIGUIENTE LINEA PARA GUARDAR CUALQUIER ROL//
                //Set<PartyGroup> duplicateSuppliersWithName = repo.getPartyGroupByGroupNameAndRoleType(groupName, "SUPPLIER");
                Set<PartyGroup> duplicateSuppliersWithName = repo.getPartyGroupByGroupNameAndRoleType(groupName, roleTypeId);                
                ...
                if (duplicateSuppliersWithName.size() > 0 && !"Y".equals(forceComplete)) {
                    ...
                    Map<String, Object> results = ServiceUtil.returnError(UtilMessage.expandLabel("PurchCreateSupplierDuplicateCheckFail", UtilMisc.toMap("partyId", partyGroup.getPartyId()), locale));
            ...
            Map<String, Object> input = createPartyGroup.makeValid(context, "IN");
            ...
            serviceResults = dispatcher.runSync("createPartyRole", UtilMisc.toMap("partyId", supplierPartyId, "roleTypeId", roleTypeId, "userLogin", userLogin));
            if (ServiceUtil.isError(serviceResults)) return serviceResults;
            ...
            if (UtilValidate.isNotEmpty(generalAddress1)) {
                ...
                serviceResults = dispatcher.runSync("createPartyContactMechPurpose", input);
                if (ServiceUtil.isError(serviceResults)) return serviceResults;
                input = UtilMisc.toMap("partyId", supplierPartyId, "userLogin", userLogin, "contactMechId", contactMechId, "contactMechPurposeTypeId", "PO_DELIV_ADDRESS");
                serviceResults = dispatcher.runSync("createPartyContactMechPurpose", input);
                if (ServiceUtil.isError(serviceResults)) return serviceResults;
