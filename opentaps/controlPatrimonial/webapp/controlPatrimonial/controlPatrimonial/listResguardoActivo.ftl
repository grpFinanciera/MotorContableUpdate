...
<@paginate name="listResguardoActivoFijo" list=activoFijoListBuilder rememberPage=false>    
    <#noparse>    
        <@navigationHeader/>
        <table class="listTable">
            <tr class="listTableHeader">
				...
				<@headerCell title=uiLabelMap.ControlUbicacionFisica orderBy="descripcion"/>
            </tr>
            <#list pageRows as row>
            <tr class="${tableRowClass(row_index)}">
                <@displayLinkCell text=row.fixedAssetId?if_exists href="verResguardoActivoFijo?fixedAssetId=${row.fixedAssetId?if_exists}"/>
