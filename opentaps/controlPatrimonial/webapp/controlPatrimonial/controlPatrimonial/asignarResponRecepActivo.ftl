		...
		{	confirmar=confirm("\u00bfEst\u00e1 seguro?"); 
			...
			{	var facilityId = document.getElementById('facilityId'+contador).value;
		    	...
		    	{ 	var requestData = {'facilityId' : facilityId, 'partyId' : partyId};		    			    
		    		opentaps.sendRequest('eliminaResponsablePermisoAlmacen', requestData, function(data) {responseElimina(data)});
		    	}				
			}													
		}
...
<@frameSection title=uiLabelMap.ControlResponsableRecepcionAlmacenActivoFijo> 
<form method="post" action="<@ofbizUrl>asignarResponsablePermisoAlmacen</@ofbizUrl>" name="asignarResponsablePermisoAlmacen">
...
<form name="listResponsablesRecepcionAlmacenForm" method="post" action="">
  <table width=100%>
	<td align="center"><font size=2><b>${uiLabelMap.ControlResponsableNotificaciones}</b></font></td>	
  </table width=100%></td>
