...
public final class FinancialServices {
    ...
    /** Clases para buscar los datos del estado de actividades. */
    public static final List<String> ESTADO_ACTIVDADES = Arrays.asList("REVENUE", "EXPENSE");
    ...
    public static Map getBalanceSheetForTimePeriod(DispatchContext dctx, Map context) {
        ...
        String glFiscalTypeId = (String) context.get("glFiscalTypeId");
		String opcionBusca = (String) context.get("opcionBusca");//Se agrega este parametro para el reporte del activo
        ...
        try {
            ...
            if (("ACTUAL".equals(glFiscalTypeId)) && (isClosed)) {
            	...
            	Debug.logWarning("opcionBusca  ["+opcionBusca+"]", MODULE);
            	
            	if(opcionBusca != null && opcionBusca.equals("BUSCAACTIVO")){
    ...
    public static Map getTrialBalanceForDate(DispatchContext dctx, Map context) {
        ...
        try {
            LedgerRepositoryInterface ledgerRepository = dl.getDomainsDirectory().getLedgerDomain().getLedgerRepository();
    ...
    public static Map getBalanceSheetForDate(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            if ("ACTUAL".equals(glFiscalTypeId) && lastClosedTimePeriod != null) {
				...
				input.put("opcionBusca", opcionBusca);
    ...
    public static Map getComparativeBalanceSheet(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            Map input = UtilMisc.toMap("organizationPartyId", organizationPartyId, "glFiscalTypeId", glFiscalTypeId1, "asOfDate", fromDate, "userLogin", userLogin,"opcionBusca", opcionBusca);
            ...
            input = UtilMisc.toMap("organizationPartyId", organizationPartyId, "glFiscalTypeId", glFiscalTypeId2, "asOfDate", thruDate, "userLogin", userLogin,"opcionBusca", opcionBusca);
            UtilAccountingTags.addTagParameters(context, input);
    ...
    public static Map getIncomeStatementAccountSumsByTagByDate(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            DomainsDirectory domainsDirectory = domainsLoader.getDomainsDirectory();
    ...
    /**
     * Obtiene el periodo anterior de un periodo dado
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String,Object> obtenPeriodoAnterior(DispatchContext dctx, Map context) {
    	
    	Delegator delegator = dctx.getDelegator();
    	String customTimePeriodId = context.get("customTimePeriodId").toString();
    	String organizationPartyId = (String) context.get("organizationPartyId");
    	String customPeriodFromId = "";
    	
		try {
    	
    	if(customTimePeriodId != null){
        	//Datos necesarios para obtener el periodo inmediato anterior de la busqueda (saldo inicial)
            
	            //Primero buscamos el periodo obtenido de pantalla
	            
	            GenericValue periodo = delegator.findByPrimaryKey("CustomTimePeriod", UtilMisc.toMap("customTimePeriodId",customTimePeriodId));

	            
	            Debug.logWarning("periodo    [  "+periodo+" ]", MODULE);
	            //obtenemos la fecha mas vieja "from"
	            java.sql.Date tsPeriodo = periodo.getDate("fromDate");
	            
	            Debug.logWarning("periodo  fromDate  [  "+tsPeriodo+" ]", MODULE);
	            ...
	            EntityCondition conditions = EntityCondition.makeCondition(EntityOperator.AND,
	                    EntityCondition.makeCondition("organizationPartyId", EntityOperator.EQUALS, organizationPartyId),
	                    EntityCondition.makeCondition("thruDate", EntityOperator.LESS_THAN_EQUAL_TO, tsPeriodo),
	                    EntityCondition.makeCondition("periodTypeId",EntityOperator.EQUALS,periodTypeId));
	            
	            List<GenericValue> timePeriods = delegator.findList("CustomTimePeriod", conditions,
	                    UtilMisc.toSet("customTimePeriodId", "periodTypeId", "isClosed", "fromDate", "thruDate"),
	                    UtilMisc.toList("thruDate DESC"), null, false);            
	            
	            Debug.logWarning("closedTimePeriods  [  "+timePeriods+" ]"+"   TAMAÑO   "+timePeriods.size(), MODULE);
	            
	            if(timePeriods!= null && timePeriods.size() > 0 ){
	            	
	            	customPeriodFromId = timePeriods.get(0).getString("customTimePeriodId");
	            	
	            	 Debug.logWarning("customPeriodFromId  [  "+customPeriodFromId+" ]", MODULE);
    	...
    	result.put("customPeriodFromId", customPeriodFromId);
    ...
    /**
     * De una lista de cuentas se obtienen los movimientos de debito y credito de cada cuenta se regresa una lista con los debitos y otra con los creditos 
     * @param dctx
     * @param context
     * @return
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
	public static Map agregarTransacciones(DispatchContext dctx, Map context) {
    	
    	Delegator delegator = dctx.getDelegator();
    	Map input = new HashMap(context);
    	String organizationPartyId = (String) input.get("organizationPartyId");
    	String glFiscalTypeId = (String) input.get("glFiscalTypeId");
    	java.sql.Timestamp fecIni = (java.sql.Timestamp) input.get("fecIni");
    	java.sql.Timestamp fecFin = (java.sql.Timestamp) input.get("fecFin");
    	
    	Debug.logWarning("organizationPartyId  [  "+organizationPartyId+" ]", MODULE);
    	Debug.logWarning("glFiscalTypeId  [  "+glFiscalTypeId+" ]", MODULE);
    	Debug.logWarning("fecIni  [  "+fecIni+" ]", MODULE);
    	Debug.logWarning("fecFin  [  "+fecFin+" ]", MODULE);
    	
		Map<String, BigDecimal> cuentasDebito = new HashMap<String, BigDecimal>();
		Map<String, BigDecimal> cuentasCredito = new HashMap<String, BigDecimal>();
		
    	
        try {
	    	
			List<GenericValue> assetAccounts = EntityUtil.orderBy(((Map<GenericValue, BigDecimal>) input.get("assetAccountBalances")).keySet(), UtilMisc.toList("glAccountId"));
	    	List<String> idAcc = FastList.newInstance();
	    	
	    	for (GenericValue genericValue : assetAccounts) {
	    		idAcc.add(genericValue.getString("glAccountId"));
			}
	    	
	        EntityCondition conditions = EntityCondition.makeCondition(EntityOperator.AND,
	                EntityCondition.makeCondition("glAccountId", EntityOperator.IN, idAcc),
	                EntityCondition.makeCondition("organizationPartyId", EntityOperator.EQUALS, organizationPartyId),
	                EntityCondition.makeCondition("glFiscalTypeId", EntityOperator.EQUALS, glFiscalTypeId),
	                EntityCondition.makeCondition("transactionDate", EntityOperator.LESS_THAN_EQUAL_TO, fecFin),
	                EntityCondition.makeCondition("transactionDate", EntityOperator.GREATER_THAN_EQUAL_TO, fecIni));
	
			List<GenericValue> cuentasTransacc = delegator.findByCondition("AcctgTransEntrySums", conditions, UtilMisc.toList("glAccountId","debitCreditFlag","amount"), UtilMisc.toList("glAccountId"));
			
			Debug.logWarning("cuentasTransacc  [  "+cuentasTransacc+" ]", MODULE);
			
			for (GenericValue ctasTrans : cuentasTransacc) {
				
				String tipo = ctasTrans.getString("debitCreditFlag");
				String ctaId = ctasTrans.getString("glAccountId");
				BigDecimal monto = ctasTrans.getBigDecimal("amount");
				
				if(tipo.equals("D")){
					
					cuentasDebito.put(ctaId, monto);
					
				} else {
					
					cuentasCredito.put(ctaId, monto);
    	...
    	Map<String, Object> result = ServiceUtil.returnSuccess();
    	result.put("cuentasDebito", cuentasDebito);
    	result.put("cuentasCredito", cuentasCredito);
    ...
    /**
     * Generates estado de situacion financiera as of a particular date, SIN TOMAR EN CUENTA LOS PERIODOS CERRADOS,
     *  or, if none, from the beginning of the earliest time period.s
     * @param dctx a <code>DispatchContext</code> value
     * @param context a <code>Map</code> value
     * @return a <code>Map</code> value
     */
    @SuppressWarnings("unchecked")
    public static Map getEdoSituacionFinancieraForDate(DispatchContext dctx, Map context) {
        LocalDispatcher dispatcher = dctx.getDispatcher();
        Delegator delegator = dctx.getDelegator();
        String organizationPartyId = (String) context.get("organizationPartyId");
        Timestamp asOfDate = (Timestamp) context.get("asOfDate");
        ...
        String glFiscalTypeId = (String) context.get("glFiscalTypeId");
        String customTimePeriodId = (String) context.get("customTimePeriodId");
        String opcionBusca = (String) context.get("opcionBusca");
...
//            	desdeFecha = UtilDateTime.getDayEnd(UtilDateTime.toTimestamp(timePeriod.getDate("fromDate")));
            ...
            Debug.logWarning("desdeFecha    ["+desdeFecha.toString()+"]   asOfDate  ["+asOfDate.toString()+"] ", MODULE);

            // now add the new asset, liability, and equity transactions
            Map tags = UtilAccountingTags.getTagParameters(context);
            assetAccountBalances = getAcctgTransAndEntriesForClass(assetAccountBalances, organizationPartyId, desdeFecha, asOfDate, glFiscalTypeId, "ASSET", tags, true, userLogin, dispatcher);
            if(opcionBusca == null || opcionBusca.isEmpty()){
            	Debug.logWarning("Entro a buscar Liability y Equity  $ ", MODULE);
                liabilityAccountBalances = getAcctgTransAndEntriesForClass(liabilityAccountBalances, organizationPartyId, desdeFecha, asOfDate, glFiscalTypeId, "LIABILITY", tags, true, userLogin, dispatcher);
                equityAccountBalances = getAcctgTransAndEntriesForClass(equityAccountBalances, organizationPartyId, desdeFecha, asOfDate, glFiscalTypeId, "EQUITY", tags, true, userLogin, dispatcher);
            }
  
            // calculate a net income since the last closed date and add it to our equity account balances
            Map input = new HashMap(context);
            input.put("fromDate", desdeFecha);
            input.put("thruDate", asOfDate);
            input = dctx.getModelService("getIncomeStatementByDates").makeValid(input, ModelService.IN_PARAM);
            tmpResult = dispatcher.runSync("getIncomeStatementByDates", input);
            GenericValue retainedEarningsGlAccount = (GenericValue) tmpResult.get("retainedEarningsGlAccount");
            ...
            EntityCondition conditions = EntityCondition.makeCondition(EntityOperator.AND,
                    ...
                    EntityCondition.makeCondition("isClosed", EntityOperator.NOT_EQUAL, "Y"),
                    EntityCondition.makeCondition(EntityOperator.OR,
                            EntityCondition.makeCondition("fromDate", EntityOperator.GREATER_THAN_EQUAL_TO, desdeFecha),
                            EntityCondition.makeCondition("thruDate", EntityOperator.LESS_THAN_EQUAL_TO, asOfDate)));
            List timePeriods = delegator.findByCondition("CustomTimePeriod", conditions, UtilMisc.toList("customTimePeriodId"), UtilMisc.toList("customTimePeriodId"));
            if (timePeriods.size() > 0) {
                isClosed = false;
	   ...
	   /**
     * Genera es Estado de Situacion Financiera for two dates and determines the balance difference between the two. The balances are in BigDecimal.
     * The output includes the result of getBalanceSheetForDate for the fromDate and thruDate and glFiscalTypeId1 and glFiscalTypeId2.
     *
     * @param dctx a <code>DispatchContext</code> value
     * @param context a <code>Map</code> value
     ...
     */
    @SuppressWarnings("unchecked")
    public static Map getComparativeSituacionFinanciera(DispatchContext dctx, Map context){
        LocalDispatcher dispatcher = dctx.getDispatcher();
        Locale locale = UtilCommon.getLocale(context);
        ...
        Debug.logWarning("fromCustomTimePeriodId   cOMPARATIVE  "+fromPeriodId, MODULE);
        Debug.logWarning("thruCustomTimePeriodId   cOMPARATIVE  "+thruPeriodId, MODULE);

        try {
            // create the balance sheet for the fromDate
            Map input = UtilMisc.toMap("organizationPartyId", organizationPartyId, "glFiscalTypeId", glFiscalTypeId1, "asOfDate", fromDate,
            						"userLogin", userLogin,"customTimePeriodId",fromPeriodId,"opcionBusca",opcionBusca);
            UtilAccountingTags.addTagParameters(context, input);
            Map fromDateResults = dispatcher.runSync("getEdoSituacionFinancieraForDate", input);
            if (ServiceUtil.isError(fromDateResults)) {
                return UtilMessage.createAndLogServiceError(fromDateResults, "FinancialsError_CannotCreateComparativeBalanceSheet", locale, MODULE);
            }

            // create the balance sheet for the thruDate
            input = UtilMisc.toMap("organizationPartyId", organizationPartyId, "glFiscalTypeId", glFiscalTypeId2, "asOfDate", thruDate,
					"userLogin", userLogin,"customTimePeriodId",thruPeriodId,"opcionBusca",opcionBusca);
            UtilAccountingTags.addTagParameters(context, input);
            Map thruDateResults = dispatcher.runSync("getEdoSituacionFinancieraForDate", input);
            if (ServiceUtil.isError(thruDateResults)) {
                return UtilMessage.createAndLogServiceError(thruDateResults, "FinancialsError_CannotCreateComparativeBalanceSheet", locale, MODULE);
            }
...
//            UtilAccountingTags.addTagParameters(context, input);
    ...
    public static Map getEstadoDeActividades(DispatchContext dctx, Map context) {
	    ...
	    /**
     * Servicio que genera el estado de flujo de efectivo a partir de dos fechas , glFiscalTypeIds determinandno la diferencia entre ambos.
     * Las cantidades son BigDecimal. 
     *
     * @param dctx a <code>DispatchContext</code> value
     * @param context a <code>Map</code> value
     * @return a <code>Map</code> value
     */
    @SuppressWarnings("unchecked")
    public static Map getComparativeEstadoFlujoEfectivo(DispatchContext dctx, Map context) {
        LocalDispatcher dispatcher = dctx.getDispatcher();
        Locale locale = UtilCommon.getLocale(context);
        ...
        // validate the from/thru dates
        if (fromDate1.after(thruDate1) || fromDate2.after(thruDate2)) {
            return UtilMessage.createAndLogServiceError("FinancialsError_CannotCreateComparativeFlujoEfectivoFromDateAfterThruDate", locale, MODULE);
        }
        ...
        try {
            ...
            Map input = UtilMisc.toMap("organizationPartyId", organizationPartyId, "glFiscalTypeId", glFiscalTypeId1, "fromDate", fromDate1, "thruDate", thruDate1, "userLogin", userLogin);
            UtilAccountingTags.addTagParameters(context, input);
            Map set1Results = dispatcher.runSync("getEstadoFlujoEfectivoForDates", input);
            if (ServiceUtil.isError(set1Results)) {
                return UtilMessage.createAndLogServiceError(set1Results, "FinancialsError_CannotCreateComparativeFlujoEfectivo", locale, MODULE);
            }

            // create the balance sheet for the thruDate
            input = UtilMisc.toMap("organizationPartyId", organizationPartyId, "glFiscalTypeId", glFiscalTypeId2, "fromDate", fromDate2, "thruDate", thruDate2, "userLogin", userLogin);
            UtilAccountingTags.addTagParameters(context, input);
            Map set2Results = dispatcher.runSync("getEstadoFlujoEfectivoForDates", input);
            if (ServiceUtil.isError(set2Results)) {
                return UtilMessage.createAndLogServiceError(set2Results, "FinancialsError_CannotCreateComparativeFlujoEfectivo", locale, MODULE);
            }

            Map results = ServiceUtil.returnSuccess();

            // include the two income statements in the results
            results.put("set1FlujoDeEfectivo", set1Results);
            results.put("set2FlujoDeEfectivo", set2Results);

            Debug.logWarning("set1FlujoDeEfectivo     ====  "+set1Results.toString()+"  "+set1Results.size(), MODULE);
            Debug.logWarning("set2FlujoDeEfectivo     ====  "+set2Results.toString()+"  "+set2Results.size(), MODULE);
    ...
    /**
     * Busca las transacciones por cuenta para el flujo de efectivo , regresa las transacciones por grupos 
     *
     * @param dctx a <code>DispatchContext</code> value
     * @param context a <code>Map</code> value
     * @return a <code>Map</code> value
     */
    @SuppressWarnings("unchecked")
    public static Map getEstadoFlujoEfectivoForDates(DispatchContext dctx, Map context) {
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Delegator delegator = dctx.getDelegator();
        Timestamp fromDate = (Timestamp) context.get("fromDate");
	...
	/**
	 * Metodo que busca el indice en el que se encuentra la clave administrativa en la tabla de claves presupuestales
	 * de acuerdo al tipo de clave (INGRESO O EGRESO), el ciclo de busqueda se obtiene de la fecha del sistema
	 * @param delegator
	 * @param tipoClave
	 * @return
	 * @throws GenericEntityException
    ...
    public static Map buscaIndiceAdministrativa(DispatchContext dctx, Map context) throws GenericEntityException, ServiceException {
        String tipoClave = (String) context.get("tipoClave");
        Delegator delegator = dctx.getDelegator();
        
        Integer numAdmin = Integer.valueOf(-1);
        //Se busca la estructura de las claves presupuestales
		List<GenericValue> estructura = delegator
				.findByAnd("EstructuraClave", UtilMisc.toMap("ciclo", String.valueOf(Calendar.getInstance().get(Calendar.YEAR)),
						"acctgTagUsageTypeId", tipoClave));
		if(estructura != null && !estructura.isEmpty()){
			for (GenericValue estr : estructura) {
    			for(int i = 1 ; i<= 15;i++){
    				String clas = estr.getString("clasificacion"+i);
    				if(clas != null && clas.equalsIgnoreCase("CL_ADMINISTRATIVA")){
    					numAdmin = Integer.valueOf(i);
    					break;
    				}
    			}
			}
		...
		if(numAdmin.intValue() < 0)
			throw new ServiceException("No se encontro la clasificaci\u00f3n administrativa ");
		
		Map results = ServiceUtil.returnSuccess();
		results.put("posicionClasificaAdmin",numAdmin);
