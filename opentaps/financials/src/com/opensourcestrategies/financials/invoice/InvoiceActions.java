...
public final class InvoiceActions {
    ...
    public static void viewInvoice(Map<String, Object> context) throws GeneralException {
        ...
        String cicloId = UtilCommon.getCicloId(request);
        ...
        String tipoClasificacion = new String();//en esta variable se almacena el tipo de clasificacion que se gustara (EGRESO o INGRESO)
        ...
        if (invoice.isPartnerInvoice()) {
            ...
            tipoClasificacion = UtilAccountingTags.EGRESO_TAG;
            ...
            tipoClasificacion = UtilAccountingTags.INGRESO_TAG;
        }
        
        //Obtiene los enumeration o party para configurar la clave presupuestaria a afectar
        ac.put("tagTypes", UtilAccountingTags.getClassificationTagsForOrganization(organizationPartyId,cicloId, tipoClasificacion, delegator));
...
//        } 
        ...
        boolean ejercidaParcialmente = false;
        boolean devengadoParcialmente = false;        
        ...
        if(invoice.getStatusId().equalsIgnoreCase("EJERCIDO_PARCIAL"))
        {	ejercidaParcialmente = true;
        	limitedEditOnly = true;
        }
        ac.put("ejercidaParcialmente", ejercidaParcialmente);
        
        if(invoice.getStatusId().equalsIgnoreCase("DEVENGADO_PARCIAL"))
        {	devengadoParcialmente = true;
        	limitedEditOnly = true;
        }
        ac.put("devengadoParcialmente", devengadoParcialmente);        
        ...
        if (ordersList != null) {
            ...
            GenericValue orderH = delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId", ordersList));
            ac.put("ordersPagoAnti", orderH.getString("pagoAnticipado"));
            ac.put("ordersPagoAntiRest", orderH.getString("pagoAnticipadoRestante"));
        }
        ...
        String moduloId = new String();
        if(invoice.isSalesInvoice()){
        	moduloId = "TESORERIA_CXC_OR";
        } else {
        	moduloId = "TESORERIA_CXP_OR";
        }
        
        ac.put("tiposDocumento", delegator.findByAndCache("EventoContable", UtilMisc.toMap("moduloId", moduloId), UtilMisc.toList("descripcion")));
    }
    ...
    public static void findInvoices(Map<String, Object> context) throws GeneralException, ParseException {
     ...
     // build search conditions
        ...
        if (partyId != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.partyId.name(), EntityOperator.EQUALS, partyId.trim()));
        }
        if (partyIdFrom != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.partyIdFrom.name(), EntityOperator.EQUALS, partyIdFrom.trim()));
        }
        if (invoiceId != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceId.name(), EntityOperator.EQUALS, invoiceId.trim()));
        }
        if (statusId != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.statusId.name(), EntityOperator.EQUALS, statusId.trim()));
        }
        if (processingStatusId != null) {
            // this is a special case where we want an empty status
            if ("_NA_".equals(processingStatusId)) {
                search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.processingStatusId.name(), EntityOperator.EQUALS, null));
            } else {
                search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.processingStatusId.name(), EntityOperator.EQUALS, processingStatusId.trim()));
            }
        }
        String dateFormat = UtilDateTime.getDateFormat(locale);
        if (invoiceDateFrom != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceDate.name(), EntityOperator.GREATER_THAN_EQUAL_TO, UtilDateTime.getDayStart(UtilDateTime.stringToTimeStamp(invoiceDateFrom, dateFormat, timeZone, locale), timeZone, locale)));
        }
        if (dueDateFrom != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.dueDate.name(), EntityOperator.GREATER_THAN_EQUAL_TO, UtilDateTime.getDayStart(UtilDateTime.stringToTimeStamp(dueDateFrom, dateFormat, timeZone, locale), timeZone, locale)));
        }
        if (paidDateFrom != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.paidDate.name(), EntityOperator.GREATER_THAN_EQUAL_TO, UtilDateTime.getDayStart(UtilDateTime.stringToTimeStamp(paidDateFrom, dateFormat, timeZone, locale), timeZone, locale)));
        }
        if (invoiceDateThru != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceDate.name(), EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayEnd(UtilDateTime.stringToTimeStamp(invoiceDateThru, dateFormat, timeZone, locale), timeZone, locale)));
        }
        if (dueDateThru != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.dueDate.name(), EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayEnd(UtilDateTime.stringToTimeStamp(dueDateThru, dateFormat, timeZone, locale), timeZone, locale)));
        }
        if (paidDateThru != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.paidDate.name(), EntityOperator.LESS_THAN_EQUAL_TO, UtilDateTime.getDayEnd(UtilDateTime.stringToTimeStamp(paidDateThru, dateFormat, timeZone, locale), timeZone, locale)));
        }
        if (amountFrom != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceTotal.name(), EntityOperator.GREATER_THAN_EQUAL_TO, new BigDecimal(amountFrom)));
        }
        if (amountThru != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceTotal.name(), EntityOperator.LESS_THAN_EQUAL_TO, new BigDecimal(amountThru)));
        }
        if (openAmountFrom != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.openAmount.name(), EntityOperator.GREATER_THAN_EQUAL_TO, new BigDecimal(openAmountFrom)));
        }
        if (openAmountThru != null) {
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.openAmount.name(), EntityOperator.LESS_THAN_EQUAL_TO, new BigDecimal(openAmountThru)));
        }
        if (referenceNumber != null) {
            search.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD(org.opentaps.base.entities.Invoice.Fields.referenceNumber.name()), EntityOperator.LIKE, EntityFunction.UPPER("%" + referenceNumber + "%")));
        }
        if (message != null) {
            search.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD(org.opentaps.base.entities.Invoice.Fields.invoiceMessage.name()), EntityOperator.LIKE, EntityFunction.UPPER("%" + message + "%")));
        }        
        ...
        if (enableFindByOrder && orderId != null) {
            ...
            search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceId.name(), EntityOperator.IN, invoiceIds));
        }
        ...
        // required conditions
        search.add(EntityCondition.makeCondition(org.opentaps.base.entities.Invoice.Fields.invoiceTypeId.name(), EntityOperator.EQUALS, invoiceTypeId.trim()));


        // Pagination
        Set<String> fieldsToSelect = new HashSet<String>(new Invoice().getAllFieldsNames());
        fieldsToSelect.retainAll(new org.opentaps.base.entities.Invoice().getAllFieldsNames());
        EntityListBuilder invoiceListBuilderInvoice = new EntityListBuilder(repository, org.opentaps.base.entities.Invoice.class, EntityCondition.makeCondition(search, EntityOperator.AND), fieldsToSelect, UtilMisc.toList(org.opentaps.base.entities.Invoice.Fields.invoiceDate.desc()));
        PageBuilder<org.opentaps.base.entities.Invoice> pageBuilderInvoice = new PageBuilder<org.opentaps.base.entities.Invoice>() {
            public List<Map<String, Object>> build(List<org.opentaps.base.entities.Invoice> page) throws Exception {
                Delegator delegator = ac.getDelegator();
                List<Map<String, Object>> newPage = FastList.newInstance();
                for (org.opentaps.base.entities.Invoice invoice : page) {
                    ...
                    newRow.put("createdStamp", invoice.getCreatedStamp());
        ...
        invoiceListBuilderInvoice.setPageBuilder(pageBuilderInvoice);

        ac.put("invoiceListBuilderInvoice", invoiceListBuilderInvoice);
    }
}
