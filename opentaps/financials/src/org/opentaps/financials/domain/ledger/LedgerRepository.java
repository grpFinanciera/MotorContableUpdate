...
public class LedgerRepository extends Repository implements LedgerRepositoryInterface {
    ...
    /** {@inheritDoc} */
	public List<AcctgTrans> getTransactionsC(String acctgId) throws RepositoryException {
    	
    	List<AcctgTrans> ListAcctgTrans = FastList.newInstance();
    	try {  	
			Debug.log("MODULE: LedgerRepository C" + acctgId);
			AcctgTrans acctgTrans = findOneNotNull(AcctgTrans.class,
					map(AcctgTrans.Fields.acctgTransId, acctgId));
			ListAcctgTrans.add(acctgTrans);
		} catch (Exception e) {
			Debug.log("MODULE: LedgerRepository - Error al consultar acctgTransId [" +  acctgId + "] -excepcion  " + e);
			throw new RepositoryException(e);
	...
	/** Obtenemos idSecuencia*/
    public String getEstructuraClave(List<GenericValue> estrClaves) throws RepositoryException {
    	
    	Iterator<GenericValue> estId = estrClaves.iterator();
		GenericValue est = estId.next();
		
        return est.getString("idSecuencia");
    }
    
    /** Obtenemos partyId*/
    public String getPartyId(List<GenericValue> acctgTrans) throws RepositoryException {
    	
    	Iterator<GenericValue> estId = acctgTrans.iterator();
		GenericValue est = estId.next();
		
        return est.getString("acctgTransId");
    }
