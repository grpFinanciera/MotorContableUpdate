...
<#if datosInvoice?has_content>
	<#list datosInvoice as invo>
		<#if invo.statusId == 'INVOICE_IN_PROCESS' || invo.statusId == 'EJERCIDO_PARCIAL'>								
			<#assign LinkEnviarMotor><@submitFormLink form="enviarOrdenEjercer" text=uiLabelMap.CommonEnviar /></#assign>													
		</#if>
	</#list>
</#if>
<@form name="enviarOrdenEjercer" id="enviarOrdenEjercer" url="enviarOrdenEjercer" >
<#assign moneda = "" />
<@frameSection title=uiLabelMap.FinancialsCrearOrdenPago extra=LinkEnviarMotor>	
		<table>								
			<#if datosInvoice?has_content>
				<#list datosInvoice as invoice>	 
					...
					<tr>
					    ...
					    <@inputDateTimeCell name="fechaPago"  />
  					</tr>
					<@displayRow title=uiLabelMap.FinancialsIdProveedor text=invoice.partyIdFrom/>
					...
					<td>
						<th>
				        	<hr><@displayTooltip text=uiLabelMap.FinancialsEventoParaPago />
					  	</th>
					</td>
					<@inputSelectRow title=uiLabelMap.FinancialsNombreEventoParaPagado name="acctgTransTypeId" list=eventos?sort_by("descripcion") required=false id="acctgTransTypeId" key="acctgTransTypeId" displayField="descripcion" onChange="cambiaEvento(this.value,'10');"/>																	
				</#list>
	    	</#if>	    		
		</table>
</@frameSection>
