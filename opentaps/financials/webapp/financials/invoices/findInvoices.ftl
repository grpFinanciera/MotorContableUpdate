...
<form method="post" action="<@ofbizUrl>${formTarget}</@ofbizUrl>" name="findInvoiceForm" style="margin: 0pt;">
  ...
  <table class="fourColumnForm">
    <tbody>
      <@inputTextRow title=uiLabelMap.FinancialsPreOrden name="invoiceId" size="20" maxlength="20"/>
      ...
      <tr>
        ...
        <@inputCurrencyRangeCell fromName="amountFrom" thruName="amountThru" defaultCurrencyUomId="MXN"/>

        <@displayTitleCell title=uiLabelMap.OpentapsOpenAmount />
        <@inputCurrencyRangeCell fromName="openAmountFrom" thruName="openAmountThru" defaultCurrencyUomId="MXN"/>
      </tr>

      <tr>
        <@displayTitleCell title=uiLabelMap.FinancialsReferenciaFactura />
        <@inputTextCell name="referenceNumber" size="30"/>

        <#if enableFindByOrder>
          <@displayTitleCell title=uiLabelMap.FinancialsOrdenCompra />
