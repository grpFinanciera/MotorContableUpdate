...
<@frameSection title="${uiLabelMap.AccountingPayment} ${payment?has_content?string(payment.paymentId,'')}" extra=paymentStatusChangeAction?if_exists>
    <table class="twoColumnForm" style="border:0">
      ...
      <@displayRow title=uiLabelMap.Documento text=payment.idTipoDoc?if_exists />
      <@displayRow title=uiLabelMap.Banco text=payment.bancoId?if_exists />
      <@displayRow title=uiLabelMap.CuentaBancaria text=payment.cuentaBancariaId?if_exists />
...
<#if hasAmountToApply && hasApplyPermission && !(allocatePaymentTagsToApplications && (payment.isReceived() || payment.isSent()))>
    ...
    <#else>
      ...
      <div class="frameSectionBody">${screens.render("component://financials/widget/financials/screens/common/PaymentScreens.xml#InvoicePaymentApplicationListCustom")}</div>
      <br/>
    </#if>
</#if>
<#--SE COMENTA PARA OCULTAR LA FUNCIONALIDAD DE SELECCIONAR LA CUENTA CONTABLE CON LA QUE SE HAR� UN PAGO-->
<#--<#if hasAmountToApply && hasApplyPermission && !(allocatePaymentTagsToApplications && (payment.isReceived() || payment.isSent()))>
