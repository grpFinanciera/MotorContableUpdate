...
<table class="twoColumnForm" style="border:none">
  ...
  <#else>
    <@inputSelectRow title=uiLabelMap.AccountingPaymentType name="paymentTypeId" ignoreParameters=true list=paymentTypeList?default([]) key="paymentTypeId" displayField="description" default="${paymentTypeId?if_exists}"/>    
  </#if>

  <@inputSelectRow title=uiLabelMap.AccountingTipoDocumento name="acctgTransTypeId" list=eventos?default([]) key="acctgTransTypeId" displayField="descripcion"/>     
  ...
  <#if paymentMethodList?exists && paymentMethodList?has_content && ( fromParty?exists || isDisbursement )>
    <tr>
      ...
      <#else>
        <td nowrap="nowrap">
          <select name="paymentMethodId" id="paymentMethodId" class="selectBox" onchange="checkPaymentMethodTypeId(this.form)">
  ...
  <tr>
    ...
    <#else>
      <td nowrap="nowrap">
        <input type="text" class="inputBox" name="amount" value="<#if paymentValue?has_content>${paymentValue.amount?if_exists}<#elseif parameters.amount?exists>${parameters.amount}<#elseif totalAmount?exists>${totalAmount}</#if>"/>
  ...
  <tr>
    ...
    <#else>
      <td nowrap="nowrap">
        ...
        <#else>
          <@inputDateTime name="effectiveDate" form=formAction default=nowTimestamp/>
        </#if>
      </td>
    </#if>
  </tr>
  ...
  <tr>
    ...
    <tr>
        <#if paymentValue?exists>
     		<@displayTitleCell title=uiLabelMap.Banco titleClass="requiredField"/>
        	<@inputSelectCell list=bancos?if_exists displayField="nombreBanco" name="bancoId" id="bancoId" default=paymentValue.bancoId?if_exists onChange="contactTypeChanged(this.form);"/>
        <#else>	
        	<@displayTitleCell title=uiLabelMap.Banco titleClass="requiredField"/>
        	<@inputSelectCell list=bancos?if_exists displayField="nombreBanco" name="bancoId" id="bancoId" onChange="contactTypeChanged(this.form);"/>
        	<script type="text/javascript">
				opentaps.addOnLoad(contactTypeChanged());
			</script>
	 	</#if>
      ...
      <tr>      
	    ...
	    <td>  
      		<select name="cuentaBancariaId" id="cuentaBancariaId" size="1" class="selectBox">      		
		  		<#if paymentValue.cuentaBancariaId?has_content>		  		
			  		<#list cuentas as cuenta>
			        	<option  value="${cuenta.cuentaBancariaId}" <#if cuenta.cuentaBancariaId == paymentValue.cuentaBancariaId> selected='selected' </#if> >${cuenta.get("numeroCuenta",locale)} - ${cuenta.get("nombreCuenta",locale)}</option>
			        </#list>
		        </#if>		
	  		</select>		
	  	</td>	          
