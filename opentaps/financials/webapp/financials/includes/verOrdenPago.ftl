...
<script  languaje="JavaScript">    
	//Functions to send request to server for load relate objects
		...
		/*function recalcularPago(contador) 
		{	var montCont = parseFloat("0");
			var montoPresupuestal = parseFloat("0");
		  	...
		  	{	var total=parseFloat(montCont) + parseFloat(montoPresupuestal);		  				  		
		  		document.getElementById('montoPorPagar').value = total;
		  	}
		  	else	
...
<#assign montoAPagar=0 id="montoAPagar" name="montoAPagar"/>
...
<#--<#if statusId != "ORDEN_PAGO_PENDIENTE">
	<@form name="ImprimirChequePoliza" url="ImprimirChequePoliza" paymentId="269" />
	<#assign LinkEnviarMotor>${LinkEnviarMotor!}<@submitFormLink form="ImprimirChequePoliza" text=uiLabelMap.FinancialsImprimeCheque class="subMenuButton" /></#assign>
</#if>
-->

<#if ! statusId.equals("ORDEN_PAGO_PAGADA")>
		<#assign LinkEnviarMotor>${LinkEnviarMotor}<@submitFormLink form="enviarOrdenPagadoParcial" text=uiLabelMap.Pagar class="subMenuButton"/></#assign>
</#if>		
...
<form id="actualizarFechaPago" method="post" action="actualizarFechaPago">
	<table>
		<tr>
			<#if ! statusId.equals("ORDEN_PAGO_PAGADA")>			
				<input type="hidden" name="invoiceId" id="invoiceId" value="${invoiceId}"/>
				<input type="hidden" name="ordenPagoId" id="ordenPagoId" value="${ordenPagoId}"/>				
				<td><@displayTooltip text=uiLabelMap.ActualizarFechaPago /></td>
				<td>													
				    <@displayTitleCell title=uiLabelMap.FechaDePago titleClass="requiredField"/>
				    <@inputDateTimeCell name="fechaPagoValor" default=fechaPago?if_exists/>
			    </td>
			    <td><input id="submitButton" type="button" onclick="javascript:$('actualizarFechaPago').submit();" class="smallSubmit" value="${uiLabelMap.ActualizarFecha}" /></td>
			<#else>
				<@displayDateRow date=fechaPago format="DATE_TIME" title=uiLabelMap.FechaDePago/>
			</#if>    
		<tr>
	</table>	
...
<form method="post" action="<@ofbizUrl>enviarOrdenPagadoParcial</@ofbizUrl>" name="enviarOrdenPagadoParcial" style="margin: 0;" id="enviarOrdenPagadoParcial">

<@frameSection title=uiLabelMap.VerOrdenPago extra=LinkEnviarMotor>
	...
	<table width="80%">			
		<tr>
			...
			<@displayRow title=uiLabelMap.ordenPagoId text=ordenPagoId/>
			<@displayRow title=uiLabelMap.FinancialsPreOrdenPago text=invoiceId/>
			<#list datosInvoice as datos>
		    	<@displayDateRow date=datos.invoiceDate format="DATE_TIME" title=uiLabelMap.FechaContablePreOrdenPago/>
		    	<@displayRow title=uiLabelMap.DescripcionPreOrdenPago text=datos.description/>
		    	...
		    	<@displayRow title=uiLabelMap.AccountingPaymentType text=datos.descriptionPayment/>	    			    					
		    </#list>
			
			
			<#if sumaTotal?exists>
				<tr>        			
        				<@displayTitleCell title=uiLabelMap.MontoTotalOrden />
        				<@displayCurrencyCell amount=sumaTotal?if_exists currencyUomId=parameters.orgCurrencyUomId/>
        				<input type="hidden" name="montoPago" id="montoPago" value="${sumaTotal?if_exists}"/>
        			
        				<@displayTitleCell title=uiLabelMap.MontoPresupuestal />        				        				
        				<#if (sumaPresuRestante?if_exists > 0)>																																																																					
							<@inputTextCell name="montoPresupuestal" id="montoPresupuestal" default="${sumaPresuRestante}" onChange="recalcularPago();"/>							
						<#else>
           ...
           <#if !tipoMatrizId?exists>
           		<#list montoPagarContable as contable>
           			<#assign montoAPagar=montoAPagar + contable.montoRestante?if_exists/>																			 												
				</#list>
				<@displayTitleCell title=uiLabelMap.MontoAPagar />
           		<@inputTextCell name="montoPorPagar" id="montoPorPagar" default="${montoAPagar?if_exists}" readonly=true size=10/>
           <#else>
           		<#assign montoAPagar=0/>
           		<#list montoPagarContable as contable>
           			<#assign montoAPagar=montoAPagar + contable.montoRestante?if_exists/>																			 												
				</#list>
				<@displayTitleCell title=uiLabelMap.MontoAPagar />
           		<@inputTextCell name="montoPorPagar" id="montoPorPagar" default="${montoAPagar+sumaPresuRestante}" readonly=true size=10/>		
           </#if>           
           			
			<#-- VALIDA SI EXISTEN DATOS DE PAGO-->
		    
		    <#if datosPayment?has_content>		    
			    <#list datosPayment as datos>					
					<@displayRow title=uiLabelMap.AccountingPaymentType text=datos.description?if_exists/>
					<@displayRow title=uiLabelMap.CommonComments text=datos.comments?if_exists/>
					<@displayRow title=uiLabelMap.FinancialsPaymentRefNum text=datos.paymentRefNum?if_exists/>
		...
		<tr><td colspan="4">&nbsp;</td></tr>
  		...
  		<table width="80%">
			...
			<tbody>
				...
				<#list montoPagarContable as contable>														
					<tr class="${tableRowClass(contable_index)}" name="contable" >
						...
						<#if (contable.montoRestante > 0)>																																																																					
							<@inputTextCell name="montoBancoContable${count}" id="montoBancoContable${count}"default="${contable.montoRestante?if_exists}" onChange="recalcularPago('${count}');"/>
				...
				<#list montoPagarPresupuestal as presupuestal>														
...
<#if listPagosOrdenPago?has_content>	      

	<@frameSection title=uiLabelMap.FinancialsPagosRealizados>		
			<table width="100%">
					
			<thead>		
				<tr>
					<th>${uiLabelMap.FinancialsPaymentId}</th>
					<th>${uiLabelMap.PartyDesde}</th>
					<th>${uiLabelMap.PartyHacia}</th>					
			...
			<tbody>								
				<#list listPagosOrdenPago as datos>														
					<tr class="${tableRowClass(datos_index)}" name="datos" >
