      ...
      <#if acctgTrans.isPosted == "Y">
        <#list acctgTransEntries as entry>
          <tr class="${tableRowClass(entry_index)}">
            ...
            <td><#assign glAccount = entry.getRelatedOneCache("GlAccount")/><a class="linktext" href="<@ofbizUrl>AccountActivitiesDetail?glAccountId=${glAccount.glAccountId}&amp;organizationPartyId=${organizationPartyId!parameters.organizationPartyId!''}</@ofbizUrl>">${glAccount.accountCode?default(glAccount.glAccountId)}</a>: ${glAccount.accountName?default("")}</td>
