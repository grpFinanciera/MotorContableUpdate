      ...
      <#if acctgTrans.isPosted == "Y">
        <#list listEntries as entry>
          <tr class="${tableRowClass(entry_index)}">
            <td><#assign glAccount = entry.getRelatedOneCache("GlAccount")/><a class="linktext" href="<@ofbizUrl>AccountActivitiesDetail?glAccountId=${glAccount.glAccountId}&amp;organizationPartyId=${organizationPartyId!''}</@ofbizUrl>">${glAccount.accountCode?default(glAccount.glAccountId)}</a>: ${glAccount.accountName?default("")}</td>
      ...
      <#else/>
        ...
        <#if listEntries?has_content>
          ...
          <form name="updateAcctgTransEntryForm" method="POST" action="<@ofbizUrl>updateAcctgTransEntryInline</@ofbizUrl>">
            ...
            <@inputHiddenRowCount list=listEntries />
