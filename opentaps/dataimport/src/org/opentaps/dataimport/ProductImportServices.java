 ...
 */
public final class ProductImportServices extends MotorContable {
...
//                int importedFromFacility = importInventoryToFacility(facilityToImport.getString("facilityId"), inventoryGlAccountId, offsettingGlAccountId, userLogin, dispatcher);
                int importedFromFacility = importInventoryToFacility(facilityToImport.getString("facilityId"), userLogin, dispatcher);
    ...
    private static int importInventoryToFacility(String facilityId, GenericValue userLogin, LocalDispatcher dispatcher) throws GenericEntityException, GenericServiceException {
...
//        GenericValue organizationParty = facility.getRelatedOneCache("OwnerParty");
        String organizationPartyId = facility.getString("ownerPartyId");
...
//                List toStore = decodeInventory(productInventory, organizationPartyId, facilityId, inventoryGlAccountId, offsettingGlAccountId, acctgTransId, currencyUomId, now, delegator);
                List toStore = decodeInventory(productInventory, organizationPartyId, facilityId, currencyUomId, now, delegator);
                if (toStore == null) {
    ...
    private static List<GenericValue> decodeProduct(GenericValue data, Timestamp now, String goodIdentificationTypeId1, String goodIdentificationTypeId2, Delegator delegator) throws GenericEntityException, Exception {
        ...
        input.put("quantityUomId", data.get("quantityUomId"));        
        input.put("primaryProductCategoryId", data.get("primaryProductCategoryId"));
        input.put("clasificacion", data.get("clasificacion"));        
        
        input.put("codigoCucop", data.get("codigoCucop"));
        input.put("capitulo", data.get("capitulo"));
        input.put("nombreCapitulo", data.get("nombreCapitulo"));
        input.put("concepto", data.get("concepto"));
        input.put("nombreConcepto", data.get("nombreConcepto"));
        input.put("partidaGenerica", data.get("partidaGenerica"));
        input.put("nombrePartidaGenerica", data.get("nombrePartidaGenerica"));
        input.put("partidaEspecifica", data.get("partidaEspecifica"));
        input.put("nombrePartidaEspecifica", data.get("nombrePartidaEspecifica"));
        input.put("partidaGasto", data.get("partidaGasto"));
        input.put("nombrePartidaGasto", data.get("nombrePartidaGasto"));
        input.put("cabm", data.get("cabm"));
        input.put("procedencia", data.get("procedencia"));
        input.put("paisOrigen", data.get("paisOrigen"));
        input.put("gradoContenido", data.get("gradoContenido"));
        input.put("patentesDerechos", data.get("patentesDerechos"));
        input.put("modelo", data.get("modelo"));
        input.put("numeroParte", data.get("numeroParte"));
        input.put("editorial", data.get("editorial"));
        
        input.put("coleccionId", data.get("coleccionId"));
        input.put("isbn", data.get("isbn"));
        input.put("autor", data.get("autor"));
        input.put("anio", data.get("anio"));
        input.put("numPaginas", data.get("numPaginas"));
        input.put("coediciones", data.get("coediciones"));               
        
        input.put("areaId", data.get("areaId"));
        ...
        listPrice.put("productPricePurposeId", "SALE_PRICE");
        listPrice.put("price", data.get("purchasePrice"));
     ...
     */
    @SuppressWarnings("unchecked")
    private static List<GenericValue> decodeInventory(GenericValue productInventory, String organizationPartyId, String facilityId, String currencyUomId, Timestamp now, Delegator delegator) throws GenericEntityException, Exception {
        Map input;
        List toStore = FastList.newInstance();
        ...
        Timestamp fechaContable = productInventory.getTimestamp("fechaContable");
        if (UtilValidate.isEmpty(onHand)) {
            onHand = new Double(0);
        }
        
        fechaContable = (UtilValidate.isEmpty(fechaContable)?UtilDateTime.nowTimestamp(): fechaContable);
        ...
        input.put("partyId", obtenPadre(delegator, organizationPartyId));
        input.put("ownerPartyId", organizationPartyId);
        input.put("datetimeReceived", now);
        input.put("facilityId", facilityId);
        input.put("comments", "Producto Ingresado desde la Importaci\u00f3n de Datos");
        ...
        input.put("reasonEnumId", "VAR_INITIAL");
...
//        acctgTransSeqNum++;
        ...
        phistory.set("productId", productId);
        phistory.set("organizationPartyId", organizationPartyId);
        phistory.set("unitCost", averageCost);
        phistory.set("cantidad", onHand);
        phistory.set("effectiveDate", now);
