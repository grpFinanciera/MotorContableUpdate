...
<form name="createOrEditWarehouse" method="post" action="<@ofbizUrl>${target}</@ofbizUrl>">
  ...
  <table class="twoColumnForm">
	<#if facility.facilityId?exists>
		<@displayRow title=uiLabelMap.WarehouseCodigoAlmacen text=facility.facilityId/>
	</#if>
  ...
  <!--  <@inputTextRow name="squareFootage" title=uiLabelMap.ProductSquareFootage size=6 default=(facility.squareFootage)! /> -->
    <tr>
        <@displayTitleCell title=uiLabelMap.ProductFacilityOwner />
        <@inputLookupCell lookup="LookupPartyName" form="createOrEditWarehouse" name="ownerPartyId" default=(facility.ownerPartyId)! />
     </tr>
