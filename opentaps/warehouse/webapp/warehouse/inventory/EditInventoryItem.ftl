...
<#-- Se agrega para ver la poliza generada -->
<#if ShipmentReceipt?exists>
	<#assign acctgTransId = ShipmentReceipt.acctgTransId/>
	<@form name="verPoliza" url="viewAcctgTrans" acctgTransId=acctgTransId/>
	<!-- <#assign actionLink>${actionLinks!}<@submitFormLink form="verPoliza" text=uiLabelMap.FinancialVerPoliza class="subMenuButton" /></#assign> -->
	<#assign actionLink><@submitFormLink form="verPoliza" text=uiLabelMap.FinancialVerPoliza class="subMenuButton" /></#assign>
</#if>

<@frameSection title="${uiLabelMap.ProductEditInventoryItemWithId} [${inventoryItemId?if_exists}]" extra=actionLink?if_exists>
  ...
  <#else>
    <form action="<@ofbizUrl>CreateInventoryItem</@ofbizUrl>" method="post" style="margin: 0;" name="inventoryItemForm">
      ...
      <#if "NON_SERIAL_INV_ITEM" == (inventoryItem.inventoryItemTypeId)?if_exists>
        ...
        <@displayRow title=uiLabelMap.ProductAvailablePromiseQuantityHand text="${inventoryItemData.quantityOnHandTotal?if_exists}" />
