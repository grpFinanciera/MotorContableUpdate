...
public class OagisInventoryServices {
    ...
    public static Map<String, Object> oagisReceiveSyncInventory(DispatchContext ctx, Map<String, Object> context) {
        ...
        try {
            userLogin = delegator.findByPrimaryKey("UserLogin", UtilMisc.<String, String>toMap("userLoginId", "system"));
        ...
        if (UtilValidate.isNotEmpty(dataAreaList)) {
            try {
                for (Element dataAreaElement : dataAreaList) {
                    ...
                    GenericValue product = delegator.findByPrimaryKeyCache("Product", UtilMisc.<String, String>toMap("productId", productId));
                    if (product == null) {
        ...
        if (errorMapList.size() == 0 && inventoryMapList.size() > 0) {
            try {
                ...
                try {
                    facilityContactMechs = delegator.findByAnd("FacilityContactMech", UtilMisc.<String, String>toMap("facilityId", facilityId));
                } catch (GenericEntityException e) {
                ...
                while (fcmIter.hasNext()) {
                    ...
                    try {
                        contactMech = delegator.findByPrimaryKey("ContactMech", UtilMisc.<String, String>toMap("contactMechId", contactMechId));
                    } catch (GenericEntityException e) {
